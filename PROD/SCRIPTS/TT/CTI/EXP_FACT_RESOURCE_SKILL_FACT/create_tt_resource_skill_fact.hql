
CREATE EXTERNAL TABLE IF NOT EXISTS CTI.TT_RESOURCE_SKILL_FACT (
    RESOURCE_SKILL_FACT_KEY VARCHAR(20),
    START_DATE_TIME_KEY VARCHAR(20),
    END_DATE_TIME_KEY VARCHAR(20),
    TENANT_KEY VARCHAR(20),
    RESOURCE_KEY VARCHAR(20),
    SKILL_KEY VARCHAR(20),
    CREATE_AUDIT_KEY VARCHAR(20),
    UPDATE_AUDIT_KEY VARCHAR(20),
    START_TS VARCHAR(20),
    END_TS VARCHAR(20),
    SKILL_LEVEL VARCHAR(20),
    IDB_ID VARCHAR(20),
    ACTIVE_FLAG VARCHAR(20),
    PURGE_FLAG VARCHAR(20),
    ORIGINAL_FILE_NAME VARCHAR(50),
    ORIGINAL_FILE_SIZE INT,
    ORIGINAL_FILE_LINE_COUNT INT
)
COMMENT 'CTI '
ROW FORMAT DELIMITED FIELDS TERMINATED BY ';'
LOCATION '/PROD/TT/CTI/EXP_FACT_RESOURCE_SKILL_FACT_'
TBLPROPERTIES ('serialization.null.format'='')
;
