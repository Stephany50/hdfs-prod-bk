...
LEFT JOIN (select max(region) region,ci from dim.dt_gsm_cell_code group by CI) b on a.location_ci = b.ci
LEFT JOIN DIM.DT_REGIONS_MKT r ON TRIM(COALESCE(upper(b.region), 'INCONNU')) = upper(r.ADMINISTRATIVE_REGION)
WHERE TRANSACTION_DATE = '2020-04-29' and  (upper(trim(subs_benefit_name))='RP DATA SHAPE_5120K' or  subs_benefit_name  is  null or (amount_data+nvl(AMOUNT_SMS_ONNET,0) +nvl(AMOUNT_SMS_OFFNET,0)+nvl(AMOUNT_SMS_INTER,0)+nvl(AMOUNT_SMS_ROAMING,0)+nvl(AMOUNT_SVA,0) + nvl(AMOUNT_VOICE_ONNET,0) +nvl(AMOUNT_VOICE_OFFNET,0)+nvl(AMOUNT_VOICE_INTER,0)+nvl(AMOUNT_VOICE_ROAMING,0)=0)) and SUBS_AMOUNT>0
GROUP BY TRANSACTION_DATE
    ,COMMERCIAL_OFFER
    ,OPERATOR_CODE
    ,REGION_ID
