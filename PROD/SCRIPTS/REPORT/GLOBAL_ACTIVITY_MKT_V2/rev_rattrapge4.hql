...
SELECT
    ...
    where transaction_date ='2020-09-04' and KPI= 'REVENUE' AND sub_account='MAIN' --AND SOURCE_TABLE IN ('FT_A_SUBSCRIPTION','FT_GSM_TRAFFIC_REVENUE_DAILY','FT_A_GPRS_ACTIVITY','FT_OVERDRAFT','IT_ZTE_ADJUSTMENT','FT_SUBS_RETAIL_ZEBRA','FT_CREDIT_TRANSFER','FT_CONTRACT_SNAPSHOT')
    group by
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'REVENUE_OM'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'REVENUE' AND sub_account='MAIN' and (SUBSTRING(DESTINATION_CODE,1,13)='REVENUE_VOICE' or SUBSTRING(DESTINATION_CODE,1,11)='REVENUE_SMS' or source_table in('FT_SUBS_RETAIL_ZEBRA','FT_CREDIT_TRANSFER','FT_CONTRACT_SNAPSHOT') or  DESTINATION_CODE  in ('UNKNOWN_BUN'))
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and 1=0
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'VALEUR_AIRTIME'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI='PARC' and DESTINATION_CODE = 'USER_GROUP'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI='PARC' and DESTINATION_CODE = 'USER_GROSS_ADD'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI='PARC' and DESTINATION_CODE = 'USER_CHURN'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from (
        select region_id,cast(sum(rated_amount) as bigint) parcj0,max(source_table) source_table from  AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date= date_sub('2020-09-04',6)   and KPI='PARC' and DESTINATION_CODE = 'USER_GROUP' group by region_id
    )a
    left join  (
        select region_id,cast(sum(rated_amount) as bigint) parcj7 from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date= '2020-09-04'    and KPI='PARC' and DESTINATION_CODE = 'USER_GROUP' group by region_id
    ...
    FROM (
        ...
        from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
        where transaction_date ='2020-09-04'   and KPI= 'USERS_REGION_LOCATION_30Jrs'
    )a
    left join (
        SELECT sum(rated_amount) valeur,max(source_table) source_table
        FROM AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
        where transaction_date ='2020-09-04'   and KPI='PARC' and DESTINATION_CODE = 'USER_30DAYS_GROUP'
    )b
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'REVENUE' AND sub_account='MAIN' and (DESTINATION_CODE='REVENUE_DATA_BUNDLE' or DESTINATION_CODE='OM_DATA' or (DESTINATION_CODE='REVENUE_DATA_PAYGO' and service_code<>'NVX_GPRS_SVA') or source_table in ('FT_EMERGENCY_DATA','FT_DATA_TRANSFER'))
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
            ...
            SELECT
             ...
             from (
                SELECT
                 ...
                 from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
                where transaction_date ='2020-09-04'   and KPI= 'REVENUE' AND sub_account='MAIN' and (SUBSTRING(DESTINATION_CODE,1,12)='REVENUE_DATA' or DESTINATION_CODE='OM_DATA')
            )a
            left join (
                select
                cast(sum(rated_amount) as double )  valeur_b,max(source_table) source_table
                from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
                where transaction_date ='2020-09-04'   and KPI= 'USAGE'  and DESTINATION_CODE='OTARIE_DATA_USAGE'
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date = '2020-09-04'   and KPI= 'UNIQUE_DATA_USERS_1Mo_30Jrs'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date = '2020-09-04'   and KPI= 'PARC_OM_30Jrs'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'CASH_IN_OM'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'CASH_OUT_OM'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI in( 'MERCH_PAY_OM','BILL_PAY_OM')
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
   ...
   UNION ALL
    ...
    from (
        select sum(rated_amount) rated_amount ,max(source_table) source_table from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date ='2020-09-04'   and KPI in ('REFILL_SELF_TOP','DATA_VIA_OM')
    ) a
    left join (
        select  sum(rated_amount) rated_amount,max(source_table) source_table   from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date ='2020-09-04'   and KPI= 'VALEUR_AIRTIME'
     )  b
...
--    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date =  '2020-09-04'   and KPI= 'BALANCE_OM'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'POS_AIRTIME_30Js'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date   ='2020-09-04' and KPI= 'PDV_OM_ACTIF_30Jrs'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
    ...
    from (
        select sum(rated_amount) rated_amount,max(source_table) source_table from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date = '2020-09-04'   and KPI= 'SNAPSHOT_STOCK_DIST'
    ) a
    left join  (
        select avg(rated_amount) rated_amount,max(source_table) source_table from (
            select transaction_date ,sum(rated_amount)  rated_amount,max(source_table) source_table from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date BETWEEN date_sub('2020-09-04',6) and '2020-09-04'   and KPI= 'AVG_REFILL_CLIENT'
    ...
    left join  (
        select avg(rated_amount) rated_amount,max(source_table) source_table from (
            select transaction_date ,sum(rated_amount)  rated_amount,max(source_table) source_table from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date between CONCAT(SUBSTRING('2020-09-04',0,7),'-','01') and '2020-09-04'    and KPI= 'AVG_REFILL_CLIENT'
         group by transaction_date
        ) a
    ) b
    ...
    select
        ...
        sum(a.rated_amount)/sum(b.rated_amount) valeur
    from (
        select sum(rated_amount) rated_amount ,max(source_table) source_table from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date = '2020-09-04'   and KPI= 'SNAPSHOT_STOCK_CLIENT'
    ) a
    left join  (
        select avg(rated_amount) rated_amount,max(source_table) source_table from (
            select transaction_date ,sum(rated_amount)  rated_amount,max(source_table) source_table from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG where transaction_date BETWEEN date_sub('2020-09-04',6) and '2020-09-04'   and KPI= 'AVG_REFILL_CLIENT'
    ...
    from AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG a
    left join dim.spark_dt_regions_mkt_v2 b on a.region_id = b.region_id
    where transaction_date ='2020-09-04'   and KPI= 'UNIQUE_DATA_USERS'
    group by
    b.administrative_region ,
    b.commercial_region,
    source_table
