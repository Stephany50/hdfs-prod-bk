...
left join (select ci,max(region) region from (select administrative_region region , ci from VW_SDT_CI_INFO_NEW ) t group by ci) c on cast(c.ci as int)=cast(b.location_ci as int)
