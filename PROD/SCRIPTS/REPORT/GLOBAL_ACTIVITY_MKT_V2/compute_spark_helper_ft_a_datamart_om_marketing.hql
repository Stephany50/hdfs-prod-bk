...
left join (select ci,max(region) region from (select region_territoriale region , ci from DIM.SPARK_DT_GSM_CELL_CODE_MKT ) t group by ci) c on cast(c.ci as int)=cast(b.location_ci as int)
