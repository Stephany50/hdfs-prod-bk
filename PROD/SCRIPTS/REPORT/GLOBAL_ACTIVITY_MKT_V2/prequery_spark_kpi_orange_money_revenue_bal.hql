SELECT IF(T1.KPI_IS_LOAD=0 AND T2.bal>0 and T3.LAST_SITE>0 and T4.SITE_TRAFFIC>0 AND T3.INSERT_COUNT=2 AND T4.INSERT_COUNT=1 AND T7.COUNT>0 -- AND 
...
(SELECT COUNT(*) COUNT FROM MON.SPARK_FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = date_add('###SLICE_VALUE###',1)) T7