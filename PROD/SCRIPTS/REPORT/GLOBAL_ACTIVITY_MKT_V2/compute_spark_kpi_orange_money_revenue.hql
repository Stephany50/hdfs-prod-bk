...
FROM DATAMART_OM_MARKETING B2
WHERE JOUR BETWEEN date_sub('###SLICE_VALUE###',30) and  AND TO_DATE('29-02-2020','DD-MM-YYYY') AND STYLE NOT LIKE ('REC%')
...
WHERE JOUR BETWEEN date_sub('###SLICE_VALUE###',30) and '###SLICE_VALUE###' AND RE.REF_DATE in (select max(ref_date) ref_date from MON.SPARK_REF_OM_PRODUCTS where ref_date<='###SLICE_VALUE###' )  AND PRODUCT_LINE='DISTRIBUTION'AND SERVICE_TYPE NOT LIKE 'P2P%'
