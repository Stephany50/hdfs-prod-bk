...
SELECT
 ...
 left join  (
   select * from  tmp.SPARK_KPIS_REG2 where processing_date=add_months('2020-04-25',-1) and granularite='MONTHLY'
...
left join (
    ...
    select
        ...
        null axe_regionale,
        sum(valeur) valeur
    from (select * from CDR.SPARK_TT_BUDGET_REGIONAL_DE where event_date between date_sub('2020-04-25',7) and '2020-04-25' )a
    ...
    select
        ...
        null axe_regionale,
        sum(valeur) valeur
    from (select * from CDR.SPARK_TT_BUDGET_REGIONAL_GA where event_date between date_sub('2020-04-25',7) and '2020-04-25' )a
    left join dim.spark_dt_regions_mkt_v2 b on upper(a.type)=upper(b.administrative_region)
    group by
    administrative_region,
    commercial_region
    ...
    select
        ...
        sum(valeur) valeur
    from (select * from CDR.SPARK_TT_BUDGET_GLOBAL where event_date between date_sub('2020-04-25',7) and '2020-04-25' )a
    group by
        case
            when type in ('ca_global') then 'Telco (prepayé+hybrid) + OM'
            when type in ('ca_bundle','ca_paygo','sms_bundle','sms_paygo') then 'dont Voix'
...
left join (
    ...
    select
        ...
        null axe_regionale,
        sum(valeur) valeur
    from (select * from CDR.SPARK_TT_BUDGET_REGIONAL_DE where event_date between  CONCAT(SUBSTRING('2020-04-25',0,7),'-','01') and '2020-04-25' )a
    left join dim.spark_dt_regions_mkt_v2 b on upper(a.type)=upper(b.administrative_region)
    group by
    administrative_region,
    commercial_region
    ...
    select
        ...
        null axe_regionale,
        sum(valeur) valeur
    from (select * from CDR.SPARK_TT_BUDGET_REGIONAL_GA where event_date between  CONCAT(SUBSTRING('2020-04-25',0,7),'-','01') and '2020-04-25' )a
    left join dim.spark_dt_regions_mkt_v2 b on upper(a.type)=upper(b.administrative_region)
    group by
    administrative_region,
    commercial_region
    ...
    select
        ...
        sum(valeur) valeur
    from (select * from CDR.SPARK_TT_BUDGET_GLOBAL where event_date between  CONCAT(SUBSTRING('2020-04-25',0,7),'-','01') and '2020-04-25' )a
    group by
        case
            when type in ('ca_global') then 'Telco (prepayé+hybrid) + OM'
            when type in ('ca_bundle','ca_paygo','sms_bundle','sms_paygo') then 'dont Voix'
