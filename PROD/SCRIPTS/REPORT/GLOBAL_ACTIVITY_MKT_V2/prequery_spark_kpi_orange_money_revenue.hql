SELECT IF(T1.KPI_IS_LOAD=0 AND T2.CONTRACT_EXISTS>0 AND T5.OM_MARK>0 AND T4.OM_DIST>0 AND LAST_SITE>0 and SITE_TRAFFIC>0 and BAL>0,'OK','NOK')
 FROM
(SELECT COUNT(*) KPI_IS_LOAD FROM AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_V3  WHERE TRANSACTION_DATE='###SLICE_VALUE###' AND SOURCE_TABLE IN ('DATAMART_OM_MARKETING','DATAMART_OM_DISTRIB'))T1,
