...
SELECT IF(T1.KPI_IS_LOAD=0 AND T2.FT_EXISTS>0  AND LAST_SITE>0 and SITE_TRAFFIC>0 and conso_msisdn>0 AND nbs_ci>1000,'OK','NOK')
 FROM
...
(SELECT COUNT(*) FT_EXISTS ,count(distinct location_ci ) nbs_ci FROM AGG.SPARK_FT_A_SUBSCRIPTION WHERE TRANSACTION_DATE = '###SLICE_VALUE###' )T2,
