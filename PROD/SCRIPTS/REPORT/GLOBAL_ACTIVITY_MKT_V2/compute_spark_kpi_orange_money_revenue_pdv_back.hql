...
WHERE JOUR BETWEEN date_sub('###SLICE_VALUE###',29) and '###SLICE_VALUE###' AND RE.REF_DATE in (select max(ref_date) ref_date from MON.SPARK_REF_OM_PRODUCTS2 where ref_date<='###SLICE_VALUE###' )  AND PRODUCT_LINE='DISTRIBUTION'AND SERVICE_TYPE NOT LIKE 'P2P%'
)B2
