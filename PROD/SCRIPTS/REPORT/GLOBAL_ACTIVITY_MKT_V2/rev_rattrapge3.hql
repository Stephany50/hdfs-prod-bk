...
WHERE JOUR='2020-12-09'--and STYLE  IN ('RECHARGE','TOP_UP')
GROUP BY
    JOUR,
    REGION_ID
...
FROM AGG.SPARK_REGIONAL_DOM_DASHBOARD B2
LEFT JOIN DIM.DT_REGIONS_MKT r ON UPPER(nvl(region,'INCONNU')) = upper(r.ADMINISTRATIVE_REGION)
WHERE JOUR='2020-12-09' and round(position,0)=0
GROUP BY
    JOUR,
    REGION_ID
...
FROM AGG.SPARK_REGIONAL_DOM_DASHBOARD B2
LEFT JOIN DIM.DT_REGIONS_MKT r ON UPPER(nvl(region,'INCONNU')) = upper(r.ADMINISTRATIVE_REGION)
WHERE JOUR='2020-12-09' and round(position,0)=3
GROUP BY
    JOUR,
    REGION_ID
...
FROM AGG.SPARK_REGIONAL_DOM_DASHBOARD B2
LEFT JOIN DIM.DT_REGIONS_MKT r ON UPPER(nvl(region,'INCONNU')) = upper(r.ADMINISTRATIVE_REGION)
WHERE JOUR='2020-12-09' and round(position,0)=9
GROUP BY
    JOUR,
    REGION_ID
...
FROM AGG.SPARK_REGIONAL_DOM_DASHBOARD B2
LEFT JOIN DIM.DT_REGIONS_MKT r ON UPPER(nvl(region,'INCONNU')) = upper(r.ADMINISTRATIVE_REGION)
WHERE JOUR='2020-12-09' and round(position,0)=8
GROUP BY
    JOUR,
    REGION_ID
