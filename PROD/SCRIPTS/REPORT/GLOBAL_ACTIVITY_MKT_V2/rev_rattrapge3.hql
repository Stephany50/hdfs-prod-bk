...
select
    ...
    when category='Leviers de croissance' then 'c. Leviers de croissance'
     ...
     case
    when kpi='Telco (prepayé+hybrid) + OM' then '1. Telco (prepayé+hybrid) + OM'
    when kpi='dont sortant (~recharges)' then '2. dont sortant (~recharges)'
    when kpi='dont Voix' then '3. dont Voix'
    when kpi='Subscriber base' then '1. Subscriber base'
    when kpi='Gross Adds' then '2. Gross Adds'
    when kpi='Churn' then '3. Churn'
    when kpi='Net adds' then '4. Net adds'
    when kpi='Tx users (30jrs) en %' then '5. Tx users (30jrs) en %'
    when kpi='Revenue Data Mobile' then '1. Revenue Data Mobile'
    when kpi='Price Per Megas' then '2. Price Per Megas'
    when kpi='Data users (30jrs, >1Mo)' then '3. Data users (30jrs, >1Mo)'
    when kpi='Tx users data(30jrs) en %' then '4. Tx users data(30jrs) en %'
    when kpi='Revenue Orange Money' then '5. Revenue Orange Money'
    when kpi='Users OM (30jrs)' then '6. Users OM (30jrs)'
    when kpi='Tx users OM(30jrs) en %' then '7. Tx users OM(30jrs) en %'
    when kpi='Cash In Valeur' then '8. Cash In Valeur'
    when kpi='Cash Out Valeur' then '9. Cash Out Valeur'
    when kpi='Payments(Bill, Merch)' then '10. Payments(Bill, Merch)'
    when kpi='Self Top UP ratio (%)' then '1. Self Top UP ratio (%)'
    when kpi='Stock total client(OM)' then '2. Stock total client(OM)'
    when kpi='Nombre de Pos Airtime actif(30jrs)' then '3. Nombre de Pos Airtime actif(30jrs)'
    when kpi='Nombre de Pos OM actif(30jrs)' then '4. Nombre de Pos OM actif(30jrs)'
    when kpi='Niveau de stock @ distributor level (nb jour)' then '5. Niveau de stock @ distributor level (nb jour)'
    when kpi='Niveau de stock @ retailer level (nb jour)' then '6. Niveau de stock @ retailer level (nb jour)'
    when kpi='Engagements' then '1. Engagements'
    when kpi='Pourcentage traité en numérique' then '2. Pourcentage traité en numérique'
    when kpi='Pourcentage appel traité par BOT' then '3. Pourcentage appel traité par BOT'
    when kpi='Durée moyenne de réponse numérique' then '4. Durée moyenne de réponse numérique'
    else kpi
end kpi,
case
