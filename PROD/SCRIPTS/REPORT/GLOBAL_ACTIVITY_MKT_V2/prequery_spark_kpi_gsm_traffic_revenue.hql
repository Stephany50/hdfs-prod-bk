...
-- ABS((main_rated_amount_valeur_j_0 / main_rated_amount_valeur_j_1) - 1) < 0.5 AND 
-- ABS((promo_rated_valeur_j_0 / promo_rated_valeur_j_1) - 1) < 0.5 AND 
-- ABS((rated_count_or_duration_valeur_j_0 / rated_count_or_duration_valeur_j_1) - 1) < 0.5 
,'OK','NOK')
 FROM
(SELECT COUNT(*) KPI_IS_LOAD FROM AGG.SPARK_FT_GLOBAL_ACTIVITY_DAILY_MKT_DG_NEW  WHERE TRANSACTION_DATE='###SLICE_VALUE###' AND JOB_NAME='COMPUTE_KPI_GSM_TRAFFIC')T1,
...
FROM AGG.SPARK_FT_GSM_TRAFFIC_REVENUE_DAILY WHERE TRANSACTION_DATE=date_sub('###SLICE_VALUE###', 1))T3