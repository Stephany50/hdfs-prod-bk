...
WHERE TRANSACTION_DATE = '###SLICE_VALUE###' AND SUBS_AMOUNT > 0
  AND NVL(UPPER(SUBS_BENEFIT_NAME),'ND') NOT LIKE 'PREPAID INDIVIDUAL FORFAIT%' and  subs_channel <>'32' and  upper(subs_channel) not like '%GOS%SDP%'
GROUP BY
    TRANSACTION_DATE
...
(
    SELECT  TRANSACTION_DATE AS DATECODE,
        ...
        OPERATOR_CODE
    FROM AGG.SPARK_FT_A_SUBSCRIPTION
    WHERE TRANSACTION_DATE ='###SLICE_VALUE###' and (subs_channel='32' or upper(subs_channel) like '%GOS%SDP%')
)T
WHERE
