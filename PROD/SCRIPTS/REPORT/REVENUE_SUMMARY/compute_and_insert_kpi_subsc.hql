...
FROM 
    (SELECT     
        ...
        ,(CASE 
            WHEN NVL(UPPER(BENEFIT_NAME),'ND') LIKE 'DATACM%'
    ...
    WHERE TRANSACTION_DATE = '###SLICE_VALUE###'  AND NVL(TOTAL_AMOUNT,0) > 0 AND NVL(UPPER(BENEFIT_NAME),'ND') not like 'PREPAID INDIVIDUAL FORFAIT%'                       
    GROUP BY  
        TRANSACTION_DATE
        ,COMMERCIAL_OFFER
        ,(CASE 
