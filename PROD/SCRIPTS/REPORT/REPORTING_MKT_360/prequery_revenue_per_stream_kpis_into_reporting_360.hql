...
(
    ...
    WHERE PROCESSING_DATE='###SLICE_VALUE###' 
        ...
        AND KPI_NAME NOT IN ('DATA_VIA_OM') 
)T1,
(
...
(SELECT COUNT(*) FT_EXISTS FROM MON.SPARK_FT_CLIENT_SITE_TRAFFIC_DAY WHERE EVENT_DATE='###SLICE_VALUE###') T4,
(SELECT COUNT(*) FT_EXISTS FROM MON.SPARK_FT_VOICE_SMS_USERS WHERE TRANSACTION_DATE='###SLICE_VALUE###') T5,
(SELECT COUNT(*) FT_EXISTS FROM AGG.SPARK_FT_A_SUBSCRIPTION WHERE TRANSACTION_DATE='###SLICE_VALUE###') T6
