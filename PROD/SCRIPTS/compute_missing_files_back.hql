...
FROM (
    SELECT  LAG(SEQUENCE, 1) OVER (ORDER BY SEQUENCE) PREVIOUS, SEQUENCE,ORIGINAL_FILE_NAME FROM (
        SELECT  CAST(SUBSTRING(ORIGINAL_FILE_NAME,-19,6) AS INT) SEQUENCE,ORIGINAL_FILE_NAME  FROM  CDR.SPARK_IT_ZTE_EMERGENCY_DATA WHERE TRANSACTION_DATE="2019-03-06" AND ORIGINAL_FILE_DATE="2019-03-06"
    )T1
)T2
