...
SELECT   TRANSACTION_DATE TRANSACTION_DATE
   ...
   , SUM (MAIN_AMOUNT-MAIN_AMOUNT*0.1925) UNTAXED_AMOUNT
   , SYSDATE INSERT_DATE
   ,'REVENUE' TRAFFIC_MEAN
   , OPERATOR_CODE OPERATOR_CODE
    FROM   MON.FT_SUBS_RETAIL_ZEBRA       --MON.FT_A_ZTE_ADJUSTMENT_DATA
    WHERE
       -- EVENT_DATE  = '10/05/2018'
        TRANSACTION_DATE  in ( select datecode
from (select distinct datecode from dim.DT_DATES
     where datecode between trunc(sysdate-p2p_vas_number_day_from) and trunc(sysdate-p2p_vas_number_day_bef)
   minus
select distinct TRANSACTION_DATE datecode from FT_GLOBAL_ACTIVITY_DAILY
    where TRANSACTION_TYPE='P2P_DATA_VAS' and TRANSACTION_DATE between trunc(sysdate-p2p_vas_number_day_from) and trunc(sysdate-p2p_vas_number_day_bef))
  )
    GROUP BY
    TRANSACTION_DATE
   ,COMMERCIAL_OFFER
   ,OPERATOR_CODE ;
      ...
      COMMIT;
END; -- P_INS_P2P_DATA_VAST_REVE
END PCK_LOAD_GLOBAL_ACTIVITY;
/
...
from TMP.tt_bdi a
where substring(trim(COMPTE_CLIENT),1,6) in  (select substr(trim(COMPTE_CLIENT),1,6) from CDR.SPARK_IT_BDI_PERS_MORALE  where original_file_date = '2020-02-23')b