insert into TMP.TT_KYC_PERS_PHY_B2C_FINAL
select
...
trim(B.ADDRESSE) AS ADRESSE,
trim(B.QUARTIER) AS QUARTIER,
trim(B.VILLE) AS VILLE,
trim(B.STATUT_BSCS) AS STATUT,
...
trim(B.numero_piece_rep_legal) AS NUMERO_PIECE_REPRESENTANT_LEGAL,
trim(B.IMEI) AS IMEI,
...
trim(B.NOM_PARENT) AS NOM_TUTEUR,
...
from  TMP.TT_KYC_PERS_PHY_B2C_COMPLETE a
) a2
) B where RANG = 1