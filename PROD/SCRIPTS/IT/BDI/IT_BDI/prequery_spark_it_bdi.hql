...
(SELECT count(*) nb_J FROM CDR.SPARK_IT_BDI_FULL WHERE original_file_date=DATE_SUB('###SLICE_VALUE###',1)) J,

(SELECT COUNT(*) nb_K FROM CDR.SPARK_IT_BDI_PERS_MORALE WHERE original_file_date=DATE_SUB('###SLICE_VALUE###',1)) K,

(SELECT COUNT(*) nb_L FROM CDR.SPARK_IT_BDI_FULL WHERE original_file_date='###SLICE_VALUE###') L
