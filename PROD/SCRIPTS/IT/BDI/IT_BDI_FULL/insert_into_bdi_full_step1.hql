--- Etapes 1 :  * Dedoublonner le CRM B2C Delta.
----            * Merge entre la BDI FULL du jour précédent et le delta CRM B2C du jour actuel.
insert into TMP.TT_KYC_BDI_FULL_ST1
SELECT
...
case when B.numero_piece_representant_legal is null or trim(B.numero_piece_representant_legal) = '' then A.numero_piece_representant_legal else B.numero_piece_representant_legal end as numero_piece_representant_lega,
...
FROM (select * from CDR.SPARK_IT_KYC_BDI_FULL where original_file_date=DATE_SUB('###SLICE_VALUE###',1)) A
FULL OUTER JOIN
(select * from (select *, row_number() over (partition by guid order by to_date(ccmoddate) desc nulls last) rang from CDR.SPARK_IT_KYC_CRM_B2C where original_file_date='###SLICE_VALUE###') where rang=1) B
