...
SELECT
	...
	NVL(CAST(CREDIT_SRC as DECIMAL(20, 2)), 0) CREDIT_SRC,
	...
	NVL(CAST(CREDIT_DEST as DECIMAL(20, 2)), 0) CREDIT_DEST,
	...
	NVL(CAST(TRANSACTION_COST as DECIMAL(20, 2)), 0) TRANSACTION_COST,
	NVL(CAST(TOTAL_AMOUNT_DEBIT as DECIMAL(20, 2)), 0) TOTAL_AMOUNT_DEBIT,
	...
	WHEN TRIM (split(TRIM(USSD_ORDER), '-')[0]) = 'TRA' THEN
		(CASE
			WHEN LENGTH (TRIM (split(TRIM(USSD_ORDER), '-')[4])) = 9 THEN
				cast(split(TRIM(USSD_ORDER), '-')[3] as DECIMAL(20, 2))
			ELSE 0
		END )
	ELSE NULL
	END ) AMOUNT,
	ORIGINAL_FILE_NAME,
	FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, 13, 10),'yyyy-MM-dd')) ORIGINAL_FILE_DATE,
...
SELECT
  ORIGINAL_FILE_NAME,
  'P2P_LOG' FILE_TYPE,
  TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, 13, 10),'yyyy-MM-dd'))) ORIGINAL_FILE_DATE,
  MAX(ORIGINAL_FILE_SIZE),
  MAX(ORIGINAL_FILE_LINE_COUNT),
  CURRENT_TIMESTAMP,
  DATE_FORMAT(TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, 13, 10),'yyyy-MM-dd'))), 'yyyy-MM') ORIGINAL_FILE_MONTH
FROM CDR.TT_P2P_LOG C
WHERE NOT EXISTS (
	SELECT 1 
	FROM RECEIVED_FILES B 
