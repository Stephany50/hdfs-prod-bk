INSERT INTO CDR.SPARK_IT_P2P_LOG PARTITION (START_DATE,FILE_DATE)
SELECT
	...
	FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, -14, 10),'yyyy-MM-dd'))  FILE_DATE
FROM CDR.TT_P2P_LOG  C
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_P2P_LOG WHERE START_DATE BETWEEN DATE_SUB(CURRENT_DATE,${hivevar:date_offset}) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
