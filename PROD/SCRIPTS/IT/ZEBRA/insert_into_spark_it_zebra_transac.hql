INSERT INTO CDR.SPARK_IT_ZEBRA_TRANSAC PARTITION (TRANSACTION_DATE,FILE_DATE)
SELECT
	TRANSFER_ID	,
	REQUEST_SOURCE,
	...
	TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(TRANSFER_DATE,'dd/MM/yyyy')))  TRANSACTION_DATE,
	TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, -12, 8),'yyyyMMdd'))) FILE_DATE
FROM CDR.TT_ZEBRA_TRANSAC C 
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_ZEBRA_TRANSAC WHERE TRANSFER_DATE BETWEEN DATE_SUB(CURRENT_DATE,${hivevar:date_offset}) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
