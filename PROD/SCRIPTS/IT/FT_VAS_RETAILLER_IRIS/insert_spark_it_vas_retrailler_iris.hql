...
SUBSTRING(ORIGINAL_FILE_NAME,-10,10) ORIGINAL_FILE_DATE  ,
CURRENT_TIMESTAMP  INSERT_DATE,
to_date(START_TIME) SDATE
FROM CDR.TT_VAS_RETAILLER_IRIS C
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_VAS_RETAILLER_IRIS  WHERE SDATE BETWEEN DATE_SUB(CURRENT_DATE,3) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
