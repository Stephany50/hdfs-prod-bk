...
SELECT
 ...
 TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(regexp_extract(CDR_NAME, '\\d{8}',0),'yyyyMMdd'))) CDR_DATE FROM CDR.TT_ZTE_CHECK_FILE
WHERE NOT EXISTS (
    ...
    AND B.FILE_TYPE = 'ZTE_CHECKFILE' AND B.ORIGINAL_FILE_NAME = C.ORIGINAL_FILE_NAME
)
;
...
WHERE NOT EXISTS (SELECT 1 FROM RECEIVED_FILES B WHERE ORIGINAL_FILE_MONTH  BETWEEN DATE_FORMAT(DATE_SUB(current_date,${hivevar:date_offset}), 'yyyy-MM')
                   AND DATE_FORMAT(current_date, 'yyyy-MM') AND B.FILE_TYPE = 'ZTE_CHECKFILE' AND B.ORIGINAL_FILE_NAME = C.ORIGINAL_FILE_NAME)
