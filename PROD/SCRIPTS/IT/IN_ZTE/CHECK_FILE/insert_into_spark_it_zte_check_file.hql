INSERT INTO CDR.SPARK_IT_ZTE_CHECK_FILE PARTITION(CDR_DATE)
SELECT
 CDR_NAME,
 ( CASE 
 ...
 TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING(ORIGINAL_FILE_NAME, -14, 8),'yyyyMMdd'))) FILE_DATE
FROM CDR.TT_ZTE_CHECK_FILE C
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_ZTE_CHECK_FILE WHERE CDR_DATE BETWEEN DATE_SUB(CURRENT_DATE,${hivevar:date_offset}) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
