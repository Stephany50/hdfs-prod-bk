INSERT INTO CDR.IT_ZTE_DATA PARTITION (START_DATE)
SELECT
 EVENT_INST_ID,
 RE_ID ,
 BILLING_NBR ,
...
WHERE NOT EXISTS (
    ...
    AND B.FILE_TYPE = 'ZTE_DATA_CDR' AND (B.ORIGINAL_FILE_NAME = IF(C.ORIGINAL_FILE_NAME LIKE '%in_pr%',C.ORIGINAL_FILE_NAME,C.FILE_TAP_ID))
);
...
WHERE NOT EXISTS (SELECT 1 FROM RECEIVED_FILES B WHERE ORIGINAL_FILE_MONTH  BETWEEN DATE_FORMAT(DATE_SUB(current_date,${hivevar:date_offset}), 'yyyy-MM')
                   AND DATE_FORMAT(current_date, 'yyyy-MM') AND B.FILE_TYPE = 'ZTE_DATA_CDR' AND (B.ORIGINAL_FILE_NAME = IF(C.ORIGINAL_FILE_NAME LIKE '%in_pr%',C.ORIGINAL_FILE_NAME,C.FILE_TAP_ID)))
