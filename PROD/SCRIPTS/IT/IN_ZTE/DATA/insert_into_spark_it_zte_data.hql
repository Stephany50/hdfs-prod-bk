INSERT INTO CDR.SPARK_IT_ZTE_DATA PARTITION (START_DATE,FILE_DATE)
SELECT
 EVENT_INST_ID,
 RE_ID ,
 BILLING_NBR ,
 ...
 FROM_UNIXTIME(UNIX_TIMESTAMP(REGEXP_EXTRACT(IF(ORIGINAL_FILE_NAME LIKE '%in_pr%',ORIGINAL_FILE_NAME,FILE_TAP_ID), '[0-9]{14}', 0),'yyyyMMddHHmmss')) ORIGINAL_FILE_DATE,
 ...
 TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(REGEXP_EXTRACT(IF(ORIGINAL_FILE_NAME LIKE '%in_pr%',ORIGINAL_FILE_NAME,FILE_TAP_ID), '[0-9]{14}', 0),'yyyyMMddHHmmss'))) FILE_DATE
FROM CDR.TT_ZTE_DATA  C
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_ZTE_DATA WHERE START_DATE BETWEEN DATE_SUB(CURRENT_DATE,3) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
