INSERT INTO CDR.SPARK_IT_ZTE_SUBSCRIPTION PARTITION (CREATEDDATE)
SELECT
  ACC_NBR ,
  CHANNEL_ID,
  SUBS_EVENT_ID	,
...
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_ZTE_SUBSCRIPTION WHERE CREATEDDATE BETWEEN DATE_SUB(CURRENT_DATE,${hivevar:date_offset}) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
