...
WHERE NOT EXISTS (SELECT 1 FROM RECEIVED_FILES B WHERE ORIGINAL_FILE_MONTH  BETWEEN DATE_FORMAT(DATE_SUB(current_date,${hivevar:date_offset}), 'yyyy-MM') AND B.FILE_TYPE = 'ZTE_SUBSCRIPTION_CDR' AND DATE_FORMAT(current_date, 'yyyy-MM') AND B.ORIGINAL_FILE_NAME = C.ORIGINAL_FILE_NAME);