...
WHERE NOT EXISTS (
    ...
    AND B.FILE_TYPE = 'ZTE_DATA_POST_CDR' AND (B.ORIGINAL_FILE_NAME = IF(C.ORIGINAL_FILE_NAME LIKE '%in_postpaid_pr%',C.ORIGINAL_FILE_NAME,C.FILE_TAP_ID))
);

INSERT INTO RECEIVED_FILES PARTITION(ORIGINAL_FILE_MONTH)
SELECT
  IF(ORIGINAL_FILE_NAME LIKE '%in_postpaid_pr%',ORIGINAL_FILE_NAME,FILE_TAP_ID) ORIGINAL_FILE_NAME,
  'ZTE_DATA_POST_CDR' FILE_TYPE,
  TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (IF(ORIGINAL_FILE_NAME LIKE '%in_postpaid_pr%',ORIGINAL_FILE_NAME,FILE_TAP_ID), -25, 8),'yyyyMMdd'))) ORIGINAL_FILE_DATE,
  MAX(IF(ORIGINAL_FILE_NAME LIKE '%in_postpaid_pr%',ORIGINAL_FILE_SIZE,CAST(ISMP_PRODUCT_ID AS INT))),
  MAX(IF(ORIGINAL_FILE_NAME LIKE '%in_postpaid_pr%',ORIGINAL_FILE_LINE_COUNT,CAST(ORIGINAL_FILE_NAME AS INT))),
  CURRENT_TIMESTAMP,
  DATE_FORMAT(TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (IF(ORIGINAL_FILE_NAME LIKE '%in_postpaid_pr%',ORIGINAL_FILE_NAME,FILE_TAP_ID), -25, 8),'yyyyMMdd'))), 'yyyy-MM') ORIGINAL_FILE_MONTH
FROM CDR.TT_ZTE_DATA_POST C
