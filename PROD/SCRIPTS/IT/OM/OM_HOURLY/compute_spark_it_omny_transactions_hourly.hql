INSERT INTO CDR.SPARK_IT_OMNY_TRANSACTIONS_HOURLY
SELECT
    SENDER_MSISDN ,
    RECEIVER_MSISDN ,
    RECEIVER_USER_ID ,
    ...
    TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, 22, 8),'yyyyMMdd'))) FILE_DATE,
    CONCAT (TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, 22, 8),'yyyyMMdd'))),' ',SUBSTRING(ORIGINAL_FILE_NAME, 31, 4)) FILE_TIME
FROM CDR.SPARK_TT_OMNY_TRANSACTIONS_HOURLY C
    LEFT JOIN (SELECT DISTINCT original_file_name FILE_NAME FROM  CDR.SPARK_IT_OMNY_TRANSACTIONS_HOURLY) T ON T.file_name = C.original_file_name WHERE  T.file_name IS NULL
