INSERT INTO CDR.SPARK_IT_OMNY_ACCOUNT_SNAPSHOT PARTITION (ORIGINAL_FILE_DATE)
SELECT
    USER_NAME,
    LAST_NAME,
...
FROM CDR.TT_OMNY_ACCOUNT_SNAPSHOT_SUBSCRIBER C
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.SPARK_IT_OMNY_ACCOUNT_SNAPSHOT WHERE ORIGINAL_FILE_DATE BETWEEN DATE_SUB(CURRENT_DATE,3) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
