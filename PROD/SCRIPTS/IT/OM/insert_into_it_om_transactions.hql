INSERT INTO CDR.IT_OM_TRANSACTIONS PARTITION (TRANSACTION_DATE)
SELECT
	SENDER_MSISDN,
	RECEIVER_MSISDN,
	RECEIVER_USER_ID,
	...
	TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(TRANSFER_DATETIME,'dd/MM/yyyy HH:mm:ss'))) TRANSACTION_DATE
FROM CDR.TT_OM_TRANSACTIONS C
WHERE NOT EXISTS (
    ...
    AND B.FILE_TYPE = 'OM_TRANSACTIONS' AND B.ORIGINAL_FILE_NAME = C.ORIGINAL_FILE_NAME
);

INSERT INTO RECEIVED_FILES PARTITION(ORIGINAL_FILE_MONTH)
SELECT 
  ORIGINAL_FILE_NAME,
  'OM_TRANSACTIONS' FILE_TYPE,
  TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, -12, 8),'yyyyMMdd'))) ORIGINAL_FILE_DATE,
...
WHERE NOT EXISTS (SELECT 1 FROM RECEIVED_FILES B WHERE ORIGINAL_FILE_MONTH  BETWEEN DATE_FORMAT(DATE_SUB(current_date,${hivevar:date_offset}), 'yyyy-MM') 
                   AND DATE_FORMAT(current_date, 'yyyy-MM') AND B.FILE_TYPE = 'OM_TRANSACTIONS' AND B.ORIGINAL_FILE_NAME = C.ORIGINAL_FILE_NAME)
