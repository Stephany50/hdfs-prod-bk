INSERT INTO CDR.IT_OMNY_SERVICES_CHARGES_DETAILS PARTITION (TRANSACTION_DATE)
SELECT
	TRANSACTION_ID,
...
FROM CDR.TT_OMNY_SERVICES_CHARGES_DETAILS C
LEFT JOIN (SELECT DISTINCT  ORIGINAL_FILE_NAME FILE_NAME FROM CDR.IT_OMNY_SERVICES_CHARGES_DETAILS WHERE TRANSACTION_DATE BETWEEN DATE_SUB(CURRENT_DATE,${hivevar:date_offset}) AND CURRENT_DATE )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
