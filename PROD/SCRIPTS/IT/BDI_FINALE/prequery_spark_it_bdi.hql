...
(SELECT COUNT(*) it_bdi_crm_b2c_EXIST FROM CDR.SPARK_it_bdi_crm_b2c WHERE period= '###SLICE_VALUE###' )E,
(SELECT COUNT(*) it_bdi_hlr_EXISTS FROM CDR.spark_it_bdi_hlr WHERE sdate='###SLICE_VALUE###') F,
(SELECT COUNT(*) IT_BDI_ZSMART_EXISTS FROM CDR.SPARK_IT_BDI_ZSMART  WHERE period='###SLICE_VALUE###')G,
...
(SELECT distinct  original_file_size ZSMART_SIZE_IS_OK FROM CDR.SPARK_IT_BDI_ZSMART  WHERE period='###SLICE_VALUE###') J,
