INSERT INTO CDR.SPARK_IT_CRA_MSC_HUAWEI PARTITION (CALLDATE,FILE_DATE)
SELECT
  ...
  TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(SUBSTRING (ORIGINAL_FILE_NAME, 9, 6),'ddMMyy'))) FILE_DATE
FROM CDR.TT_CRA_MSC_HUAWEI C
LEFT JOIN (SELECT ORIGINAL_FILE_NAME FILE_NAME, count(*) nbre_ligne FROM CDR.SPARK_IT_CRA_MSC_HUAWEI WHERE CALLDATE BETWEEN DATE_SUB(CURRENT_DATE,2) AND CURRENT_DATE group by ORIGINAL_FILE_NAME having count(*)>65000  )T ON T.FILE_NAME=C.ORIGINAL_FILE_NAME
