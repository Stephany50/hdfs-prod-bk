...
(
   ...
   FROM IN_FT_01_SUBSCRIPTION ITSUBSC
    ...
    LEFT JOIN (SELECT NVL(PRICE_PLAN_CODE, 'UNKNOWN')PRICE_PLAN_CODE , MIN( PRICE_PLAN_NAME) PRICE_PLAN_NAME FROM CDR.IT_ZTE_PRICE_PLAN_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' GROUP BY NVL(PRICE_PLAN_CODE, 'UNKNOWN')) PRICE_PLAN ON NVL(ITSUBSC.PRICE_PLAN_CODE, '1000000') = PRICE_PLAN.PRICE_PLAN_CODE
    LEFT JOIN (SELECT NVL(PRICE_PLAN_CODE, 'UNKNOWN')PRICE_PLAN_CODE, MIN(PRICE_PLAN_NAME) PRICE_PLAN_NAME FROM CDR.IT_ZTE_PRICE_PLAN_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' GROUP BY NVL(PRICE_PLAN_CODE, 'UNKNOWN')) OLD_PRICE_PLAN ON NVL(ITSUBSC.OLD_PRICE_PLAN_CODE, '1000000') = OLD_PRICE_PLAN.PRICE_PLAN_CODE
    LEFT JOIN (SELECT STD_CODE, MIN(PROD_SPEC_NAME) PROD_SPEC_NAME FROM CDR.IT_ZTE_PROD_SPEC_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' GROUP BY STD_CODE) PROD_SPEC ON NVL(ITSUBSC.PROD_SPEC_CODE, '1000000') =  PROD_SPEC.STD_CODE
    LEFT JOIN (SELECT STD_CODE, MIN(PROD_SPEC_NAME) PROD_SPEC_NAME FROM CDR.IT_ZTE_PROD_SPEC_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' GROUP BY STD_CODE) REL_PROD ON NVL(ITSUBSC.RELATED_PROD_CODE, '1000000') = REL_PROD.STD_CODE
    LEFT JOIN (SELECT STD_CODE, MIN(PROD_SPEC_NAME) PROD_SPEC_NAME FROM CDR.IT_ZTE_PROD_SPEC_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' GROUP BY STD_CODE) OLD_PROD_SPEC ON NVL(ITSUBSC.OLD_PROD_SPEC_CODE, '1000000') =  OLD_PROD_SPEC.STD_CODE
