INSERT INTO MON.SPARK_FT_ACCOUNT_ACTIVITY PARTITION(EVENT_DATE)
SELECT
    a.access_key  MSISDN,
    b.og_call OG_CALL,
...
FROM
  ...
  LEFT JOIN (SELECT FN_FORMAT_MSISDN_TO_9DIGITS(MSISDN) MSISDN9, * FROM   MON.SPARK_FT_OG_IC_CALL_SNAPSHOT
  WHERE  event_date = '###SLICE_VALUE###') b ON (a.access_key = b.MSISDN9)

  LEFT JOIN (SELECT FN_FORMAT_MSISDN_TO_9DIGITS(MSISDN) MSISDN9, * FROM   MON.SPARK_FT_ACCOUNT_ACTIVITY
