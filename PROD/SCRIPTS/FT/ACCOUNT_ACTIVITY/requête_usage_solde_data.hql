...
CREATE TABLE tmp.cbm_usage_solde
-- TABLE EVALUEE A J-1
SELECT DISTINCT
...
'2023-07-10' AS event_date
-- select *
FROM
( 
SELECT DISTINCT acct_id, acc_nbr FROM CDR.SPARK_IT_ZTE_SUBS_EXTRACT WHERE original_file_date = DATE_ADD('2023-07-10', 1) 
...
WHERE original_file_date = '2023-07-10'
and acct_id in (select acct_id FROM CDR.SPARK_IT_ZTE_SUBS_EXTRACT WHERE original_file_date = DATE_ADD('2023-07-10', 1) and acc_nbr like '%699948312%' limit 1) 
) A1 
...
exp_date                 
FROM CDR.SPARK_IT_ZTE_BAL_EXTRACT
WHERE original_file_date = DATE_ADD('2023-07-10', 1)
and acct_id in (select acct_id FROM CDR.SPARK_IT_ZTE_SUBS_EXTRACT WHERE original_file_date = DATE_ADD('2023-07-10', 1) and acc_nbr like '%699948312%' limit 1) 
) A1
...
WHERE EVENT_DATE  = DATE_ADD('2023-07-10', 1)
...
( SELECT fn_format_msisdn_to_9digits(SERVED_PARTY_MSISDN) AS MSISDN, transaction_date, transaction_time, 
transactionsn, 
bal_id ACCT_RES_ID_LIST, 
benefit_balance_list BENEFIT_BALANCE_LIST, 
service_list SERVICE_LIST,
rated_amount MD_PACC, 
...
WHERE TRANSACTION_DATE = '2023-07-10'
...
WHERE upper(B.acct_res_name) like "%DATA%" or (B.acct_res_id = 1 and upper(B.acct_res_rating_service_code) like '%DATA%')
;
