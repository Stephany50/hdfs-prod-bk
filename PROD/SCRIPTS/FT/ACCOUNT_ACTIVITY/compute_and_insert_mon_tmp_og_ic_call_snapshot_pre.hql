INSERT  INTO MON.TMP_OG_IC_CALL_SNAPSHOT_PRE
SELECT
	T.MSISDN,
...
(
	...
	(
		...
		WHERE
			TRANSACTION_DATE = DATE_SUB('2019-07-02',1)
			AND (CASE
	...
	FROM
		MON.FT_SUBSCRIPTION
	WHERE
		TRANSACTION_DATE BETWEEN TO_DATE(DATE_SUB('###SLICE_VALUE###',1)) and DATE_SUB('###SLICE_VALUE###',3)
		AND RATED_AMOUNT > 0
	GROUP BY
	...
	from
		MON.ft_cra_gprs
	where
		session_date BETWEEN TO_DATE(DATE_SUB('###SLICE_VALUE###',1)) and TO_DATE(DATE_SUB('###SLICE_VALUE###',3))
