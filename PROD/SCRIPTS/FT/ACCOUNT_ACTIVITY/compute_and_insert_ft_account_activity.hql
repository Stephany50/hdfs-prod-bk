INSERT INTO AGG.FT_ACCOUNT_ACTIVITY PARTITION(EVENT_DATE)
SELECT
    a.access_key  MSISDN,
    b.og_call OG_CALL,
...
FROM
	...
	LEFT JOIN (SELECT FN_FORMAT_MSISDN_TO_9DIGITS(MSISDN) MSISDN9, * FROM   AGG.FT_Account_activity
...
FROM
	(
		...
		FROM AGG.FT_ACCOUNT_ACTIVITY    t1
	...
	INNER JOIN( SELECT FN_FORMAT_MSISDN_TO_9DIGITS(MSISDN)MSISDN9,* FROM AGG.FT_ACCOUNT_ACTIVITY WHERE EVENT_DATE = DATE_SUB('###SLICE_VALUE###', 1) ) c   ON (a.MSISDN = c.MSISDN9)
