...
--create table TMP.TT_BALANCE_AGEE as
SELECT CODE_CLIENT, CATEGORIE, NOM, CASE WHEN count(case when prod_state='Active' then accnbr end) !=0 THEN 'ACTIF'
    WHEN count(case when prod_state in ('Inactive', 'One-Way Block', 'Two-Way Block') then accnbr end) = 0 then 'SUSPENDU' ELSE 'DESACTIVE' END STATUT,
BILLCYCLE_CODE, BALANCE, DERNIERE_FACTURE, balance_J, balance_J_30, balance_J_60, balance_J_90, balance_J_120, balance_J_150, balance_J_180,
 ...
 count(case when prod_state='Active' then accnbr end) nbre_actif, count(case when prod_state in ('Inactive', 'One-Way Block', 'Two-Way Block') then accnbr end) nbre_inactif,
 count(case when prod_state in ('Non-provisioning', 'Termination') then accnbr end) nbre_suspendu, CURRENT_TIMESTAMP INSERT_DATE, EVENT_DATE
...
LEFT JOIN (
    ...
    FROM CDR.SPARK_IT_CONT WHERE original_file_date ='2020-04-21'
) b on a.CODE_CLIENT = b.RESP_PAYMENT and rn_cont=1
WHERE event_date ='2020-04-21'
