...
SELECT a.customer_id, custcode, prgcode, ccname, ccline2, ccline3,  cast(ohstatus as string) ohstatus, ohentdate, ohrefnum, balance, billcycle,
    (CASE WHEN b.customer_id IS NOT NULL THEN balance ELSE 0 END) AS balance_prev_period, balance balance_current,
    (CASE WHEN ohrefnum IS NOT NULL THEN ohentdate ELSE NULL END) AS bill_date, a.original_file_date event_date, current_timestamp insert_date
FROM (
