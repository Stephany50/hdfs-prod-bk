...
event_date
from
(select P1.sender_msisdn numero, 
P1.nbre_trans_om nbre_transanctions_om,
...
P1.event_date
from 
(select sender_msisdn,count(transfer_id) nbre_trans_om,transfer_datetime event_date from cdr.spark_it_omny_transactions
where transfer_datetime="###SLICE_VALUE###"  group by sender_msisdn,event_date )P1
LEFT JOIN
(select count(*) nbre_achats_forfaits_om, numero from
(select trim(transfer_id) id_transfer_1,sender_msisdn numero ,transfer_datetime_nq from cdr.spark_it_omny_transactions where transfer_datetime="###SLICE_VALUE###" and upper(trim(transfer_status))='TS' 
...
LEFT JOIN
(select sender_msisdn,count(transfer_id) nbre_achat_om from cdr.spark_it_omny_transactions
where transfer_datetime="###SLICE_VALUE###" and trim(upper(service_type)) in ('MERCHPAY','SUBMERPAY') group by sender_msisdn)P3
...
minus
(select trim(transactionsn) from CDR.SPARK_IT_ZTE_SUBSCRIPTION where WHERE CREATEDDATE = "###SLICE_VALUE###")
)
)p group by sender_msisdn)P4
ON P1.sender_msisdn=P4.sender_msisdn
LEFT JOIN
(select F1.numero, F1.duree_min_depot_fortait, F1.duree_max_depot_fortait,F1.duree_moyenne_depot_fortait,F2.duree_mediane_depot_fortait from
(select numero,min(duree)duree_min_depot_fortait , max(duree) duree_max_depot_fortait, avg(duree) duree_moyenne_depot_fortait from
(select P5.sender_msisdn numero ,abs(round(unix_timestamp(P5.transfer_datetime_nq, 'yyyy-MM-dd HH:mm:ss')-unix_timestamp(P4.nq_createddate, 'yyyy-MM-dd HH:mm:ss'))) duree from
...
and fn_format_msisdn_to_9digits(receiver_msisdn) in (select trim(msisdn) from dim.ref_compte_afd))P5
inner join (select trim(transactionsn) id_transfer_2,active_date,nq_createddate from CDR.SPARK_IT_ZTE_SUBSCRIPTION where WHERE CREATEDDATE = "###SLICE_VALUE###")P4
on P4.id_transfer_2 = P5.id_transfer_1)
group by numero)F1

INNER JOIN 
(SELECT numero, avg(duree) as duree_mediane_depot_fortait
...
LEFT JOIN
(select F1.sender_msisdn, F1.heure_min_echec_achat_forfait,F1.heure_moyenne_echec_achat_forfait,F1.heure_max_echec_achat_forfait,F2.heure_mediane_echec_achat_forfait from
(select 
sender_msisdn,
substr(min(transfer_datetime_nq),12,9)heure_min_echec_achat_forfait , substr(max(transfer_datetime_nq),12,9) heure_max_echec_achat_forfait, substr(FROM_UNIXTIME(AVG(UNIX_TIMESTAMP(transfer_datetime_nq, 'yyyy-MM-dd HH:mm:ss')),'yyyy-MM-dd HH:mm:ss'),12,9)heure_moyenne_echec_achat_forfait
from
(select sender_msisdn,transfer_datetime_nq 
from cdr.spark_it_omny_transactions
where transfer_datetime="###SLICE_VALUE###"
and transfer_id in
(select trim(transfer_id) from cdr.spark_it_omny_transactions where transfer_datetime="###SLICE_VALUE###" and upper(trim(transfer_status))='TS' 
...
INNER JOIN 
(SELECT sender_msisdn,  substr(FROM_UNIXTIME(AVG(UNIX_TIMESTAMP(transfer_datetime_nq, 'yyyy-MM-dd HH:mm:ss')),'yyyy-MM-dd HH:mm:ss'),12,9) as heure_mediane_echec_achat_forfait
