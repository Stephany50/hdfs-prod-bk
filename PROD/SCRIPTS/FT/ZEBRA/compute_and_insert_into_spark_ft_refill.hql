INSERT INTO MON.SPARK_FT_REFILL
SELECT
    ZEBRA.TRANSFER_ID         REFILL_ID
     ...
     , (CASE WHEN NVL(TRANSFER_AMOUNT, 0) = 0 THEN ZEBRA.MAIN_BUNDLE_VAL
             ELSE (NVL(TRANSFER_AMOUNT,0))/100
    END) REFILL_AMOUNT
     ...
     , CAST(ZEBRA.OTHER_COMMISION AS DOUBLE)/100  COMMISSION
     , ZEBRA.TRANSFER_DATE       REFILL_DATE
     , '###SLICE_VALUE###' FILE_DATE
FROM (
         SELECT MAX(TRANSFER_ID) TRANSFER_ID
              ...
              ,MAX(TRANSFER_AMOUNT) TRANSFER_AMOUNT
              ...
              ,ID
              ,MAX(ZEBRA.OTHER_COMMISION) OTHER_COMMISION
         FROM(
                 SELECT
                     TRANSFER_ID
                      ,TRANSFER_DATE_TIME
         ...
         LEFT JOIN (SELECT SUBSTR(TRIM(MAX(PROFILE_NAME)), 1, 25) OFFER_NAME,
                   ...
                   ON IF (CAST(NVL(ZEBRA.SERVICE_CLASS_CODE, 'UNKNOW') AS INT) IS NULL,-1,CAST(ZEBRA.SERVICE_CLASS_CODE AS INT))=OFFER.STD_CODE
GROUP BY
    ZEBRA.TRANSFER_ID
    ...
    END)
       ...
       , ZEBRA.TRANSFER_DATE
       ,CAST(ZEBRA.OTHER_COMMISION AS DOUBLE)/100
