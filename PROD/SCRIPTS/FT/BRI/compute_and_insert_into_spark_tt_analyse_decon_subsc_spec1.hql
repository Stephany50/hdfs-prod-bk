insert into MON.SPARK_TT_ANALYSE_DECON_SUBSC_SPEC1
 ...
 ,ACTIVATION_DATE
 ,TRUNC(abs(cast(months_between(GP_STATUS_DATE, ACTIVATION_DATE)as int))) AGE
 ,MAX(DATE_FORMAT(OG_CALL,'yyyymm')) LAST_OUTGOING_CALL_MONTH
 ,DATE_FORMAT(GREATEST(IC_CALL_1,IC_CALL_2,IC_CALL_3,IC_CALL_4),'yyyy-MM') LAST_INCOMING_CALL_MONTH
 ...
 AND EVENT_DATE =date_sub(last_day(substring('###SLICE_VALUE###',1,7) || '-01'),-1)
 GROUP BY EVENT_DATE
 , MSISDN
 ,FORMULE
 ,GP_STATUS_DATE
 ,ACTIVATION_DATE
 ,TRUNC(abs(cast(months_between(GP_STATUS_DATE, ACTIVATION_DATE)as int)))
 ,DATE_FORMAT(GREATEST(IC_CALL_1,IC_CALL_2,IC_CALL_3,IC_CALL_4),'yyyy-MM'),PLATFORM_STATUS
 ) a
 LEFT JOIN
 ( select MSISDN
 , SITE_NAME
 ...
 WHERE EVENT_MONTH BETWEEN date_format(ADD_MONTHS('###SLICE_VALUE###' || '-01' , -5),'yyyy-MM') AND '###SLICE_VALUE###'
 ...
 WHERE smonth = date_format('###SLICE_VALUE###','yyyyMM')
 ) e
 ...
 LEFT JOIN (
 SELECT substring(e.imei,1,8) tac_code_handset, e.*, f.* FROM (SELECT a.* , ROW_NUMBER() OVER(PARTITION BY MSISDN ORDER BY TOTAL_DAYS_COUNT DESC) AS IMEI_RN FROM MON.SPARK_FT_IMEI_TRAFFIC_MONTHLY a WHERE smonth = date_format(ADD_MONTHS('###SLICE_VALUE###' || '-01' , -3),'yyyyMM')) e LEFT JOIN (select a.*, (CASE WHEN UMTS = 'O' or GPRS = 'O' or EDGE = 'O' or LTE = 'O' THEN 'YES' ELSE 'NO' END) DATA_COMPATIBLE, (CASE WHEN GPRS = 'O' or EDGE = 'O' or EDGE = 'E' THEN 'YES' ELSE 'NO' END) DATA2G_COMPATIBLE ,(CASE WHEN UMTS = 'O' or LTE = 'O' THEN 'YES' ELSE 'NO' END) DATA3G_COMPATIBLE , (CASE WHEN LTE = 'O' THEN 'YES' ELSE 'NO' END) DATA4G_COMPATIBLE from dim.spark_dt_handset_ref a ) f ON substring(e.imei,1,8) = f.tac_code WHERE e.IMEI_RN = 1
