SELECT IF((COMPLETUDE=0 or '###SLICE_VALUE###'='2020-12-31') AND NB_FILES>150000000  AND T3.SOURCES>=3,'OK','NOK')  FROM
(SELECT COUNT(*) NB_FILES FROM CDR.SPARK_IT_CRA_MSC_HUAWEI WHERE CALLDATE = '###SLICE_VALUE###') T1,
...
FROM CDR.SPARK_IT_CRA_MSC_HUAWEI WHERE CALLDATE = '###SLICE_VALUE###')T3
