...
TRANSACTION_TIME,
MAX(TRANSACTION_DIRECTION),
MAX(TRANSACTION_TYPE),
SERVED_MSISDN,
SERVED_IMSI,
MAX(SERVED_IMEI),
MAX(SERVED_PARTY_LOCATION),
MAX (CASE WHEN subs_type <> 'ROAM' AND ( LENGTH(Other_Party) <= 6 OR Other_Party IN ('99900500', '99900999')) THEN 'PREP/POST'
 ELSE subs_type
 END ) SUBSCRIBER_TYPE,
OTHER_PARTY,
MAX (CASE
...
TRANSACTION_VOLUME,
MAX(MEASUREMENT_UNIT),
MAX(TRANSACTION_TERM_CODE),
MAX(TRUNCK_IN),
MAX(TRUNCK_OUT),
MAX(MSC_ADRESS),
MAX(MSC_SOURCE_FILE),
...
OLD_CALLED_NUMBER,
ROAMING_NUMBER,
SUM(1) INITIAL_DOUBLON_COUNT,
MAX(RAW_GSMSCFADDR),
MAX(RAW_USERTYPE),
MAX(RAW_LEVELCAMELSVC),
MAX(OTHER_PARTY_TRANSLATED_NUM),
NETCALLREF,
CALLER_PFLAG,
CALLED_PFLAG,
RN,
TRANSACTION_DATE FROM (
 ...
 , NVL(CAUSEFORTERM, 0) TERM_TYPE
 , NULL MSC_TYPE
 , MSCADDRESS CRA_MSC_ID
 , GLOBALAREAID MS_LOCATION
 , SERVEDIMEI IMEI
 , OUTTRUNKGROUP TRUNCK_OUT
 , INTRUNKGROUP TRUNCK_IN
 , SOURCE CRA_FICHIER
 , NULL type_heure
 ...
 AND SERVEDIMSI = '624020447286267'
 AND DATE_FORMAT(NQ_CALLDATE,'HHmmss') = '000932'
 ) T1
 ) T2
) T3
GROUP BY
