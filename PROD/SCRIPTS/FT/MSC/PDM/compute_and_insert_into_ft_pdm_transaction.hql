...
FROM(
         -- la photo DU JOUR
        SELECT  DATE_SUB('###SLICE_VALUE###',0)  EVENT_DATE,  CASE WHEN LENGTH(SERVED_MSISDN) = 13 AND substr(SERVED_MSISDN,1,3) = '160' AND FN_GET_NNP_MSISDN_SIMPLE_DESTN(SERVED_MSISDN) 
                IN ('MTN','VIETTEL','OCM') THEN SUBSTR(SERVED_MSISDN,-9) ELSE SERVED_MSISDN END MSISDN
        ...
        GROUP BY DATE_SUB('###SLICE_VALUE###',0), CASE WHEN LENGTH(SERVED_MSISDN) = 13 AND substr(SERVED_MSISDN,1,3) = '160' AND FN_GET_NNP_MSISDN_SIMPLE_DESTN(SERVED_MSISDN) 
