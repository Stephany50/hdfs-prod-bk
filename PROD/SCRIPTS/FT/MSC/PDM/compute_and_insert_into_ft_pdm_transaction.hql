...
FROM(
         -- la photo DU JOUR
        SELECT  DATE_SUB('###SLICE_VALUE###',0)  EVENT_DATE,  CASE WHEN LENGTH(SERVED_MSISDN) = 13 AND substr(SERVED_MSISDN,1,3) = '160' AND MON.FN_NNP_SIMPLE_DESTINATION(SERVED_MSISDN) 
        ...
        GROUP BY DATE_SUB('###SLICE_VALUE###',0), CASE WHEN LENGTH(SERVED_MSISDN) = 13 AND substr(SERVED_MSISDN,1,3) = '160' AND MON.FN_NNP_SIMPLE_DESTINATION(SERVED_MSISDN) 
