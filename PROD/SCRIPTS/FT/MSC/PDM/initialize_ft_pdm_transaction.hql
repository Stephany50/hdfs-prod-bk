...
SELECT '2019-01-01' --cast(CURRENT_TIMESTAMP as date)  EVENT_DATE
    ,CASE WHEN LENGTH(SERVED_MSISDN) = 13 AND substr(SERVED_MSISDN,1,3) = '160' AND MON.FN_NNP_SIMPLE_DESTINATION(SERVED_MSISDN) 
...
WHERE --- début filtrage de données ---
    TRANSACTION_DATE between  DATE_SUB('2019-01-01',90) and DATE_SUB('2019-01-01',0)
...
GROUP BY CASE WHEN LENGTH(SERVED_MSISDN) = 13 AND substr(SERVED_MSISDN,1,3) = '160' AND MON.FN_NNP_SIMPLE_DESTINATION(SERVED_MSISDN) 
