...
create temporary function FN_NNP_SIMPLE_DESTINATION as 'cm.orange.bigdata.udf.GetNnpMsisdnSimpleDestn';
...
SELECT
    'FT_SMSC_TRANSACTION_A2P' SRC_TABLE,
    ...
    NVL(TRANSACTION_DATE, TRANSACTION_BILLING_DATE) STATE_DATE,
...
FROM
    ...
    LEFT JOIN (SELECT GLOBAL_TITLE, MIN(CARRIER) CARRIER FROM DIM.DT_SERVICE_CENTER GROUP BY GLOBAL_TITLE) D ON(PARTNER_GT = GLOBAL_TITLE)
WHERE
    TRANSACTION_BILLING_DATE = '2019-01-05'
...
GROUP BY
    ...
    NVL(TRANSACTION_DATE, TRANSACTION_BILLING_DATE),
