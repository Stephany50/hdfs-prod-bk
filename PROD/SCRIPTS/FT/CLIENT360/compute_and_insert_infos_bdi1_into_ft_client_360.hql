...
LEFT JOIN (SELECT COMMERCIAL_OFFER,OSP_CONTRACT_TYPE,OSP_ACCOUNT_TYPE,SEGMENTATION,OSP_STATUS,ACCESS_KEY FROM (SELECT * FROM MON.FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = '2019-09-22') A LEFT JOIN DIM.DT_OFFER_PROFILES B ON A.COMMERCIAL_OFFER=B.PROFILE_CODE) C ON SUBSTR(A.MSISDN, -9, 9) = C.ACCESS_KEY
