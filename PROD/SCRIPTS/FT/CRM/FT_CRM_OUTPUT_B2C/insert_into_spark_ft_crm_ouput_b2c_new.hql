...
select max(NOM),max(PRENOM),a.COMPTE_B2C Code_Client
        ...
        ,max(case 
              ...
              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) > 0 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 1000  and nvl(DISTRIBUTEUR,0)=0 then 20
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) > 0 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 1000  and nvl(DISTRIBUTEUR,0)=0 then 21
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) > 0 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 1000  and nvl(DISTRIBUTEUR,0)=0 then 22
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) > 0 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 1000  and nvl(DISTRIBUTEUR,0)=1 then 23
              --

              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 1000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 5000  and nvl(DISTRIBUTEUR,0)=0 then 30
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 1000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 5000  and nvl(DISTRIBUTEUR,0)=0 then 31
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 1000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 5000  and nvl(DISTRIBUTEUR,0)=0 then 32
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 1000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 5000  and nvl(DISTRIBUTEUR,0)=1 then 33
              --

              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 5000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 10000 and nvl(DISTRIBUTEUR,0)=0 then 40
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 5000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 10000  and nvl(DISTRIBUTEUR,0)=0 then 41
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 5000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 10000  and nvl(DISTRIBUTEUR,0)=0 then 42
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 5000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 10000  and nvl(DISTRIBUTEUR,0)=1 then 43
              --

              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 10000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 20000 and nvl(DISTRIBUTEUR,0)=0 then 50
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 10000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 20000  and nvl(DISTRIBUTEUR,0)=0 then 51
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 10000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 20000  and nvl(DISTRIBUTEUR,0)=0 then 52
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 10000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 20000  and nvl(DISTRIBUTEUR,0)=1 then 53
              --

              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 20000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 40000 and nvl(DISTRIBUTEUR,0)=0 then 60
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 20000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 40000  and nvl(DISTRIBUTEUR,0)=0 then 61
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 20000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 40000  and nvl(DISTRIBUTEUR,0)=0 then 62
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 20000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 40000  and nvl(DISTRIBUTEUR,0)=1 then 63
              --

              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 40000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 80000 and nvl(DISTRIBUTEUR,0)=0 then 70
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 40000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 80000  and nvl(DISTRIBUTEUR,0)=0 then 71
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 40000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 80000  and nvl(DISTRIBUTEUR,0)=0 then 72
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 40000 and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) < 80000  and nvl(DISTRIBUTEUR,0)=1 then 73
              --
              
              when nvl(CONTRACT_TYPE,'PURE PREPAID')='PURE PREPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 80000 and nvl(DISTRIBUTEUR,0)=0 then 80
              when CONTRACT_TYPE='HYBRID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 80000  and nvl(DISTRIBUTEUR,0)=0 then 81
              when CONTRACT_TYPE='PURE POSTPAID' and nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 80000  and nvl(DISTRIBUTEUR,0)=0 then 82
              when nvl(voix,0)+nvl(sms,0)+nvl(gprs,0)+nvl(c.REV_OM,0) >= 80000  and nvl(DISTRIBUTEUR,0)=1 then 83 ELSE 10       
