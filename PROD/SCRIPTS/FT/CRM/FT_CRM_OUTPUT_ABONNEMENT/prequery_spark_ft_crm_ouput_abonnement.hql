...
(SELECT COUNT(DISTINCT ORIGINAL_FILE_DATE) AS CDR_NBR FROM CDR.SPARK_IT_CRM_ABONNEMENTS WHERE ORIGINAL_FILE_DATE BETWEEN TO_DATE('###SLICE_VALUE###' || '-01') AND LAST_DAY(TO_DATE('###SLICE_VALUE###' || '-01'))) F4