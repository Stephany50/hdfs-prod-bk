...
-- 	 , SUM (DURATION) TEL_DURATION
	 ...
	 , SUM (CASE WHEN SERVICE_CODE IN ('NVX_SMS', 'VOI_VOX') AND DEST_OPERATOR IN ('IN_ROAM_MT') THEN (MAIN_RATED_AMOUNT + PROMO_RATED_AMOUNT) ELSE 0 END ) ROAM_IN_VOICE_REVENUE --INROAM_TOTAL_CONSO
	 , SUM (CASE WHEN SERVICE_CODE IN ('VOI_VOX') AND DEST_OPERATOR IN ('OUT_ROAM_MO') THEN NVL(MAIN_RATED_AMOUNT,0) ELSE 0 END )  ROAM_OUT_VOICE_REVENUE --ROAM_MAIN_TEL_CONSO
	 , SUM (CASE WHEN SERVICE_CODE IN ('NVX_SMS') AND DEST_OPERATOR IN ('IN_ROAM_MT') THEN MAIN_RATED_AMOUNT ELSE 0 END )  ROAM_IN_SMS_REVENUE --INROAM_SMS_CONSO
	 , SUM (CASE WHEN SERVICE_CODE IN ('NVX_SMS') AND DEST_OPERATOR IN ('OUT_ROAM_MO') THEN MAIN_RATED_AMOUNT ELSE 0 END )  ROAM_OUT_SMS_REVENUE --ROAM_SMS_CONSO
	 , SUM (CASE    WHEN  a.SERVICE_CODE = 'VOI_VOX' THEN  MAIN_RATED_AMOUNT  ELSE  0    END ) AS  MAIN_RATED_TEL_AMOUNT --CONSO_TEL_MAIN
	 , SUM (CASE WHEN SERVICE_CODE IN ('VOI_VOX') AND DEST_OPERATOR IN ('IN_ROAM_MT') THEN NVL(MAIN_RATED_AMOUNT,0) ELSE 0 END ) MAIN_RATED_TEL_ROAM_IN_AMOUNT --INROAM_MAIN_TEL_CONSO
	 , SUM (CASE WHEN SERVICE_CODE IN ('VOI_VOX') AND DEST_OPERATOR IN ('OUT_ROAM_MO') THEN NVL(MAIN_RATED_AMOUNT,0) ELSE 0 END )  MAIN_RATED_TEL_ROAM_OUT_AMOUNT --ROAM_MAIN_TEL_CONSO
	 ,SUM (CASE WHEN SERVICE_CODE IN ('VOI_VOX') AND DEST_OPERATOR IN ('OUT_SVA','OUT_CCSVA') THEN MAIN_RATED_AMOUNT ELSE 0 END )  MAIN_RATED_TEL_SVA_AMOUNT --SVA_BILLED_TEL_CONSO,
	 , SUM (CASE WHEN SERVICE_CODE IN ('VOI_VOX') AND DEST_OPERATOR NOT IN ('OUT_NAT_MOB_OCM','OUT_NAT_MOB_MTN','OUT_NAT_MOB_CAM','OUT_SVA','OUT_CCSVA','OUT_NAT_MOB_MVO','OUT_ROAM_MO','IN_ROAM_MT','OUT_NAT_MOB_NEX') THEN NVL(MAIN_RATED_AMOUNT,0) ELSE 0 END )  MAIN_RATED_TEL_INT_AMOUNT --INTERNATIONAL_MAIN_TEL_CONSO
	 ,(SUM (MAIN_RATED_AMOUNT) - (SUM (CASE    WHEN  a.SERVICE_CODE = 'VOI_VOX' THEN  MAIN_RATED_AMOUNT  ELSE  0    END ) + SUM (CASE WHEN SERVICE_CODE NOT IN ('NVX_SMS','VOI_VOX') THEN MAIN_RATED_AMOUNT ELSE 0 END ) ) ) MAIN_RATED_SMS_AMOUNT
