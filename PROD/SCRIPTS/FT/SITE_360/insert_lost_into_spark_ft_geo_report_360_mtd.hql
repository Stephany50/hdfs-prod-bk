...
(
    ...
    ( 
        ...
        WHERE EVENT_DATE between concat(substr('###SLICE_VALUE###', 1, 7), '-01') and '###SLICE_VALUE###' AND DEACTIVATION_DATE between concat(substr('###SLICE_VALUE###', 1, 7), '-01') and '###SLICE_VALUE###' 
    ) A
    LEFT JOIN
    (
        SELECT
