...
SELECT
S.MSISDN
, TO_DATE ( fn_get_sorted_list_item (CONCAT_WS('|',CAST(TO_DATE(S.OG_CALL) AS string),CAST (S.OG_CALL_COUNT AS string)),4, 'DESC', '\\|')) OG_CALL_1
 --Le plus ancien appel émis vers Orange
...
, TO_DATE ( fn_get_sorted_list_item (CONCAT_WS('|',CAST(TO_DATE(S.OG_CALL) AS string),CAST (S.OG_CALL_COUNT AS string)),1, 'DESC', '\\|'))OG_CALL_4
--Le plus récent appel émis vers Orange
, TO_DATE ( fn_get_sorted_list_item (CONCAT_WS('|',CAST(TO_DATE(S.IC_CALL) AS string),CAST (S.IC_CALL_COUNT AS string)),4, 'DESC', '\\|'))IC_CALL_1
--Le plus ancien appel  reçu d Orange
...
FROM
(SELECT
( CASE WHEN ((LENGTH (OTHER_PARTY) = 8 AND '###SLICE_VALUE###'<'2014-11-21') OR (LENGTH (FN_NNP_REMOVE_RN(OTHER_PARTY)) = 9 AND '###SLICE_VALUE###'>='2014-11-21')) THEN FN_NNP_REMOVE_RN(OTHER_PARTY) ELSE NULL  END  )  MSISDN
, MAX (CASE
...
AND fn_get_nnp_msisdn_simple_destn(OTHER_PARTY) IN ('VIETTEL', 'MTN', 'CAMTEL')
GROUP BY
( CASE WHEN ((LENGTH (OTHER_PARTY) = 8 AND '###SLICE_VALUE###'<'2014-11-21') OR (LENGTH (FN_NNP_REMOVE_RN(OTHER_PARTY)) = 9 AND '###SLICE_VALUE###'>='2014-11-21')) THEN FN_NNP_REMOVE_RN(OTHER_PARTY) ELSE NULL  END)
) S
