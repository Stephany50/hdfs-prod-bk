...
--create table JUNK.DATAMART_OM_MARKETING as
SELECT
 C.ACTIF_ID,
 C.MSISDN,
 C.SERVICE_TYPE,
...
-- CHANNEL ON RECEIVER MODE
 ...
 A.TRANSFER_STATUS='TS' AND
 A.TRANSFER_DATETIME ='2020-08-31' AND
 A.SENDER_CATEGORY_CODE='SUBS' AND A.RECEIVER_CATEGORY_CODE<>'SUBS'
 AND A.SERVICE_TYPE IN ('BILLPAY', 'CASHIN', 'CASHOUT', 'ENT2REG', 'MERCHPAY', 'P2P', 'P2PNONREG')
 GROUP BY A.TRANSFER_DATETIME, (A.SENDER_USER_ID || '_' || A.SENDER_MSISDN), A.SENDER_MSISDN, A.SERVICE_TYPE, B.STYLES,
...
UNION ALL
 ...
 'TOP_UP' PRODUCT,
 (CASE WHEN OTHER_MSISDN IS NULL THEN 'SELF' WHEN OTHER_MSISDN IS NOT NULL AND OTHER_MSISDN=SENDER_MSISDN THEN 'TIERS_SELF' ELSE 'TIERS' END) DETAILS_MARKETING,
 (CASE WHEN OTHER_MSISDN IS NULL THEN 'SELF' WHEN OTHER_MSISDN IS NOT NULL AND OTHER_MSISDN=SENDER_MSISDN THEN 'TIERS_SELF' ELSE 'TIERS' END) DETAILS_CONFOMITY,
 'TELEPHONIE' BEAC,
 COUNT(A.TRANSFER_ID) VOL,
 SUM(A.TRANSACTION_AMOUNT) VAL,
 SUM(A.SERVICE_CHARGE_RECEIVED) DIRECT_REVENUE,
 ...
 FROM CDR.SPARK_IT_OMNY_TRANSACTIONS A
 WHERE
 TRANSFER_STATUS='TS' AND TRANSFER_DATETIME ='2020-08-31' AND
SERVICE_TYPE='RC' AND
SENDER_CATEGORY_CODE='SUBS' AND
OTHER_MSISDN IS NULL
...
LEFT JOIN (SELECT * FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY where event_date='2020-08-31') D ON C.MSISDN=D.MSISDN