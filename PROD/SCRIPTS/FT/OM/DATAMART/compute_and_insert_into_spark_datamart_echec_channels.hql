...
--*****************************************
-- ***************************************
INSERT INTO MON.SPARK_DATAMART_ECHEC_CHANNELS
SELECT
 C.ACTIF_ID,
 C.MSISDN,
 C.SERVICE_TYPE,
...
-- CHANNEL ON RECEIVER MODE
 ...
 A.TRANSFER_DATETIME ='2020-09-03' AND
 A.SENDER_CATEGORY_CODE='SUBS' AND
 A.SERVICE_TYPE IN ('P2P', 'CASHOUT', 'COUTBYCODE', 'MERCHPAY', 'BILLPAY', 'INVC2C')
 GROUP BY
 A.TRANSFER_DATETIME,
...
ERROR_DESC,
 ...
 SUM(A.COMMISSIONS_OTHERS) OTHER_REVENUE,
 SUM(A.COMMISSIONS_PAID) COMMISSION
 FROM CDR.IT_OMNY_TRANSACTIONS A JOIN (select distinct MSISDN,USER_ID,STYLES,TECHNOLOGY,PRODUCT_LINE,PRODUCT,DETAILS_MARKETING,DETAILS_CONFOMITY,BEAC from backup_dwh.ref_om_products3 where to_date(REF_DATE) in (select max(to_date(ref_date)) from backup_dwh.ref_om_products3 where to_date(ref_date)<='2020-09-03') and msisdn is not null) B
 ON UPPER(A.SENDER_MSISDN)=UPPER(B.MSISDN) AND UPPER(A.SENDER_USER_ID)=UPPER(B.USER_ID)
 LEFT JOIN (select distinct MSISDN,USER_ID,STYLES,TECHNOLOGY,PRODUCT_LINE,PRODUCT,DETAILS_MARKETING,DETAILS_CONFOMITY,BEAC from backup_dwh.ref_om_products3 where to_date(REF_DATE) in (select max(to_date(ref_date)) from backup_dwh.ref_om_products3 where to_date(ref_date)<='2020-09-03') and msisdn is not null) C
...
ERROR_DESC,
 ...
 C.DETAILS_MARKETING,
 C.DETAILS_CONFOMITY,
 C.BEAC) C LEFT JOIN (SELECT * FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY where event_date='2020-09-03') D
