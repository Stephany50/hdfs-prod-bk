SELECT IF(A.nbr = 0  and B.nbr > 0 and C.nbr > 0 or D.nbr >0,'OK','NOK') FROM
...
(SELECT COUNT(*) nbr FROM cdr.spark_it_omny_transactions WHERE file_date=date_add('###SLICE_VALUE###',1))D