...
SELECT
    ...
    , TRUNC(SUM(CASE WHEN TYPE = 'CONSO VOIX SMS' THEN MAIN_DEBIT+LOAN_DEBIT ELSE MAIN_DEBIT END)/25) MARKS
    , 1 IS_COMPLETED
...
SELECT
    ...
    , TRUNC(SUM(TRANSACTION_AMOUNT)/25) MARKS
    , 1 IS_COMPLETED
    , CURRENT_TIMESTAMP() INSERT_DATE
    , '###SLICE_VALUE###' EVENT_DATE
...
SELECT
    ...
    , TRUNC(SUM(SERVICE_CHARGE_RECEIVED)/5) MARKS
    , 1 IS_COMPLETED
    , CURRENT_TIMESTAMP() INSERT_DATE
    , '###SLICE_VALUE###' EVENT_DATE
FROM CDR.SPARK_IT_OMNY_TRANSACTIONS
