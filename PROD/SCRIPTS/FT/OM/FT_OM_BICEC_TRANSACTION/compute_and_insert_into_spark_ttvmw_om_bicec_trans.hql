...
FROM   ( 
  ...
  FROM  (
                SELECT N_ROWNUM,
					   ...
					   OM_OPERATION_CODE
				FROM   (
							  SELECT ROW_NUMBER() OVER( ORDER BY SENDER_USER_ID)        N_ROWNUM ,
				...
				UNION
								 SELECT ROW_NUMBER() OVER(ORDER BY RECEIVER_USER_ID)        N_ROWNUM ,
										TO_DATE(TRANSFER_DATETIME) EVENT_DATE ,
										SERVICE_TYPE ,
...
UNION ALL
				SELECT
					  (CASE
					   ...
					   CURRENT_TIMESTAMP INSERT_DATE
				FROM   (
							  SELECT ROW_NUMBER() OVER(ORDER BY PAYER_USER_ID )       N_ROWNUM ,
				...
				UNION
								 SELECT ROW_NUMBER() OVER(ORDER BY PAYEE_USER_ID)       N_ROWNUM ,
										TO_DATE(TRANSACTION_DATE) EVENT_DATE ,
										SERVICE_TYPE ,
