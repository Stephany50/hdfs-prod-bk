...
LEFT JOIN 
    (SELECT *
     FROM CDR.SPARK_IT_OMNY_SYSSERVICETYPES
     WHERE ORIGINAL_FILE_DATE =(SELECT MAX(ORIGINAL_FILE_DATE) FROM CDR.SPARK_IT_OMNY_SYSSERVICETYPES WHERE ORIGINAL_FILE_DATE >= DATE_SUB('###SLICE_VALUE###', 7))) DT

ON OT.SERVICE_TYPE = DT.SERVICE_TYPE

WHERE OT.TRANSFER_DATETIME = '###SLICE_VALUE###' AND OT.TRANSFER_DONE LIKE '%Y%'
