...
SELECT A1.MSISDN AS MSISDN, A1.USER_ID AS USER_ID, A1.CREATION_DATE AS CREATION_DATE,A1.USER_TYPE AS USER_TYPE,
      ...
      FROM
           ( SELECT *  FROM CDR.SPARK_IT_OM_SUBSCRIBERS WHERE MODIFICATION_DATE ='2020-01-21' AND CREATION_DATE >= 1577833200) A1
      INNER JOIN (SELECT * FROM CDR.SPARK_IT_OMNY_TRANSACTIONS WHERE TRANSFER_DATETIME='2020-01-21')B1  ON A1.MSISDN = B1.SENDER_MSISDN
      ...
      FROM
            ( SELECT *  FROM CDR.SPARK_IT_OM_SUBSCRIBERS WHERE MODIFICATION_DATE ='2020-01-21' AND CREATION_DATE >= 1577833200) A2
      INNER JOIN (SELECT * FROM CDR.SPARK_IT_OMNY_TRANSACTIONS WHERE TRANSFER_DATETIME='2020-01-21')B2 ON A2.MSISDN = B2.RECEIVER_MSISDN
      ) T