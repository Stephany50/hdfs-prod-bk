...
(
  ...
  WHERE ORIGINAL_FILE_DATE IN (SELECT MAX(ORIGINAL_FILE_DATE) FROM CDR.SPARK_IT_OM_ALL_USERS  WHERE ORIGINAL_FILE_DATE BETWEEN DATE_SUB('###SLICE_VALUE###', 30) AND DATE_SUB('###SLICE_VALUE###', 1))
...
(
  ...
  WHERE EVENT_DATE IN (SELECT MAX(EVENT_DATE) FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE BETWEEN DATE_SUB('###SLICE_VALUE###', 7) AND DATE_SUB('###SLICE_VALUE###', 1))
  GROUP BY MSISDN
) D
