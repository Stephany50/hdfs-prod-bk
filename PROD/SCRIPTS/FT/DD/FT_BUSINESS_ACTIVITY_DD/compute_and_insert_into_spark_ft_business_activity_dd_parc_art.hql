...
INSERT INTO FT_BUSINESS_ACTIVITY_DD
    SELECT 'DD_PMO_PARC' usage_code,b.zone_lib,sum(effectif) parc,'FT_PARCS_SITE_DAY', CURRENT_TIMESTAMP,a.event_date
    FROM TMP.TT_PARCS_SITE_DAY a
    LEFT JOIN (select distinct zone_lib, site from mondv.dt_site_zone) b on a.site_name = b.site
    WHERE parc_type = 'PARC_ART' AND STATUT = 'ACTIF' AND  a.event_date IN 
    	(
    	 SELECT c.datecode 
    	 FROM 
