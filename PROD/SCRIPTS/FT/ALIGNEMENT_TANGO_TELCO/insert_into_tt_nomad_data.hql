...
union all
select telephone,last_update_date ORIGINAL_FILE_DATE,(CASE
WHEN trim(last_update_datetime) IS NULL OR trim(last_update_datetime) = '' THEN NULL
WHEN trim(last_update_datetime) like '%/%'
THEN  cast(translate(SUBSTR(trim(last_update_datetime), 1, 19),'/','-') AS TIMESTAMP)
WHEN trim(last_update_datetime) like '%-%' THEN  cast(SUBSTR(trim(last_update_datetime), 1, 19) AS TIMESTAMP)
ELSE NULL END) last_update_date,majle,expiration,etat,etatdexportglobal,typedecontrat
