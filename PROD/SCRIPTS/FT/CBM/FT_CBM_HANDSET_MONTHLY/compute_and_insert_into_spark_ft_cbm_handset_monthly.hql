...
(
	SELECT
		...
		, CASE WHEN TECHNOLOGIE = '5G' THEN 1 ELSE 0 END DATA_5G
	...
	(
		...
		(
			...
			left join dim.dt_handset_ref A 
			on lpad(TRIM(SUBSTR(A00.IMEI, 1, 8)), 8, 0) = lpad(TRIM(SUBSTR(A.tac_code, 1, 8)), 8, 0)
		) A
	) A0
	WHERE RANG = 1
) A
