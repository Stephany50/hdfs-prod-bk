...
INSERT INTO MON.TT_FT_CBM_DOLA_DAILY_PRE (MSISDN, DOLA, ACTIVITY_TYPE)
    ...
    WHERE TRANSACTION_DATE ='2019-06-17'
...
UNION   
    ...
    WHERE TO_DATE(PAY_TIME) = '2019-06-17'
    AND (-BILL_AMOUNT) > 0
    ...
    SELECT '2019-06-17' EVENT_DATE,MSISDN,ACTIVATION_DATE,DOLA,DOLIA,ACTIVITY_TYPE,RGSX,REGION,TOWN,QUARTER,INSERT_DATE 
    FROM MON.FT_CBM_DOLA_DAILY WHERE EVENT_DATE = date_sub('2019-06-17', 1);

    --MERGE DES DONNÉES
    MERGE INTO MON.TT_FT_CBM_DOLA_DAILY S
    USING ( SELECT  '2019-06-17' EVENT_DATE,A.MSISDN, B.ACTIVATION_DATE, A.DOLA, A.ACTIVITY_TYPE,'' RGSX,C.REGION,C.TOWN,C.QUARTER 
    FROM MON.TT_FT_CBM_DOLA_DAILY_PRE A
    LEFT JOIN    (SELECT ACCESS_KEY MSISDN,ACTIVATION_DATE 
          FROM MON.FT_CONTRACT_SNAPSHOT 
          WHERE EVENT_DATE = date_add('2019-06-17', 1)
          ) B
          ON(A.MSISDN = B.MSISDN)
    LEFT JOIN    (SELECT MSISDN,COMMERCIAL_REGION REGION  ,TOWNNAME TOWN,SITE_NAME QUARTER 
         FROM MON.FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE ='2019-06-17') C
    ...
    INSERT                
    VALUES (D.EVENT_DATE,D.MSISDN,D.ACTIVATION_DATE,D.DOLA,'', D.ACTIVITY_TYPE,D.RGSX,D.REGION,D.TOWN,D.QUARTER,CURRENT_TIMESTAMP) ;
