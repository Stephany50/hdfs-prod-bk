...
(
    ...
    WHERE EVENT_MONTH = SUBSTRING(ADD_MONTHS('###SLICE_VALUE###' || '-01', -1), 1, 7)
        AND (
            ...
            OR (LENGTH (FN_FORMAT_MSISDN_TO_9DIGITS(MSISDN)) <> 9 AND LAST_LOCATION_MONTH > TO_CHAR(ADD_MONTHS( TO_DATE (S_SLICE_VALUE || '01', 'YYYYMMDD'), -6), 'YYYYMM') )
        )
        AND IS_NUMBER(SITE_NAME)='N'
) A
FULL JOIN
(
    SELECT
        MAX (A.EVENT_MONTH) EVENT_MONTH
        , FN_FORMAT_MSISDN_TO_9DIGITS(A.MSISDN) MSISDN
        , MAX (A.SITE_NAME) SITE_NAME
        , MAX (A.EVENT_MONTH) LAST_LOCATION_MONTH
        , A.TOWNNAME
        , A.ADMINISTRATIVE_REGION
        , A.COMMERCIAL_REGION
        , A.OPERATOR_CODE
        , CURRENT_DATE() REFRESH_DATE
    FROM MON.SPARK_FT_CLIENT_SITE_TRAFFIC_MONTH A
    WHERE A.EVENT_MONTH = '###SLICE_VALUE###' AND IS_NUMBER(A.SITE_NAME)='N'
    GROUP BY FN_FORMAT_MSISDN_TO_9DIGITS(A.MSISDN), A.SITE_NAME, A.TOWNNAME, A.ADMINISTRATIVE_REGION, A.COMMERCIAL_REGION, A.OPERATOR_CODE
) B
