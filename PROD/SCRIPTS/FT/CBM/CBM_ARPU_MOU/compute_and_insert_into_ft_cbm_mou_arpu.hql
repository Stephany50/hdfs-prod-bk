...
(select
*
from  DIM.DT_CBM_REF_SOUSCRIPTION_PRICE) B on UPPER(A.bdle_name)=UPPER(B.bdle_name)
UNION
SELECT
DISTINCT '6'||SUBSTR(MSISDN, -8) AS MSISDN
FROM
...
GROUP BY CREATE_DATE, ACC_NBR , CHANNEL_ID, ACCT_RES_CODE
) A
LEFT JOIN
(
select *
...
select
DISTINCT
'6'||SUBSTR(sub_msisdn, -8) AS MSISDN
...
UNION
SELECT
DISTINCT '6'||SUBSTR(RECEIVER_MSISDN, -8) AS MSISDN
...
) A on I.msisdn=A.msisdn
left join (
select '6'||SUBSTR(MSISDN, -8) AS MSISDN ,
...
, '6'||SUBSTR(RECEIVER_MSISDN, -8)
,REFILL_MEAN
) E on I.msisdn=E.msisdn
left join(
select
'6'||SUBSTR(MSISDN, -8) AS  MSISDN,
