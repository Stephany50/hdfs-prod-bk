...
LEFT JOIN
(SELECT EVENT_DATE, MSISDN, SITE_NAME FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE= date_add( '###SLICE_VALUE###', 1)) B ON A.MSISDN=B.MSISDN AND A.PERIOD=B.EVENT_DATE
LEFT JOIN
...
LEFT JOIN
(SELECT EVENT_DATE, msisdn, site_name FROM mon.SPARK_FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE =date_add( '###SLICE_VALUE###', 1)) EN on FI.msisdn=EN.msisdn and FI.CREATE_DATE=EN.EVENT_DATE
left join
