...
create table tmp.sos_credit_update101 as
select 
            ...
            (
                ...
                (
                    select 
                        ...
                        amount loan_amount,
                        row_number() over(partition by msisdn order by cast(concat(transaction_date, ' ', concat(substr(transaction_time, 1, 2), ':', substr(transaction_time, 3, 2), ':', substr(transaction_time, 5, 2))) as TIMESTAMP) desc) rang
                    from cdr.spark_it_zte_emergency_credit
