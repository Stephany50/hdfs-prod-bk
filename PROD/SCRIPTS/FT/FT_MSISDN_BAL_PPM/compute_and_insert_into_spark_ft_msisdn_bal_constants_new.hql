...
insert into mon.spark_ft_msisdn_bal_constants_new
select
    z.msisdn
    , z.bal_id
    , (
...
(
    ...
    (
        select
            ...
            , row_number() over(partition by msisdn, bal_id order by bal_revenu desc) rang
        from mon.spark_ft_msisdn_bal_constants_new
...
where upper(d.comments) not like '%QUOTIDIEN%' -- on retire les forfaits type "giga data", "sms mensuelles", etc...
