INSERT INTO TT.TT_ZEBRA_ACTIVE_USER
SELECT A.* FROM (
SELECT sender_msisdn, sum(nbre_transaction) nbre_transaction, sum(nbre_jour)nbre_jour, sum(total_transfer_amount)total_transfer_amount, max(last_transfer_date) last_transfer_date, current_timestamp INSERT_DATE, '###SLICE_VALUE###' event_date
FROM (
...
FROM TT.TT_ZEBRA_ACTIVE_USER WHERE EVENT_DATE = DATE_SUB('###SLICE_VALUE###', 1)
UNION ALL
...
) A LEFT JOIN ( SELECT distinct EVENT_DATE FROM TT.TT_ZEBRA_ACTIVE_USER WHERE EVENT_DATE='###SLICE_VALUE###' ) b ON A.EVENT_DATE = B.EVENT_DATE
