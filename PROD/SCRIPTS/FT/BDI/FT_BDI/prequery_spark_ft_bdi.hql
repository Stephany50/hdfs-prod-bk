...
if(ft_bdi.nb_ft_bdi = 0 and it_bdi.nb_it_bdi >= 10 and ft_clsd.nb_ft_clsd >= 10 and ft_mdm.nb_ft_mdm >= 10
    and ft_dcmm.nb_ft_dcmm >= 10 and ft_dmomm.nb_ft_dmomm >= 10
            and ft_accat.nb_ft_accat >= 10 and ft_csnap.nb_ft_csnap >= 10 and it_zebm.nb_it_zebm >= 10,'OK','NOK')
FROM (select count(*) as nb_ft_bdi from MON.SPARK_FT_BDI where event_date=to_date('###SLICE_VALUE###')) ft_bdi
,(SELECT count(*) as nb_it_bdi FROM CDR.SPARK_IT_BDI where original_file_date=date_add(to_date('###SLICE_VALUE###'),1)) it_bdi
...
,(select count(*) as nb_ft_dcmm from MON.SPARK_FT_DATA_CONSO_MSISDN_MONTH where EVENT_MONTH = substr(add_months(to_date('###SLICE_VALUE###'),-1),1,7)) ft_dcmm
,(select count(*) as nb_ft_dmomm from  MON.SPARK_FT_DATAMART_OM_MONTH where EVENT_MONTH = substr(add_months(to_date('###SLICE_VALUE###'),-1),1,7)) ft_dmomm
