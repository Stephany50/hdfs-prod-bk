insert into TMP.TT_KYC_BDI_PP_ST3
SELECT
...
OR trim(A.TYPE_PERSONNE) in ('PP','MAJEUR','PDV') then trim(A.CONFORM_ART_P_PHY_MAJEUR)
...
case when trim(A.TYPE_PERSONNE) is null
OR trim(A.TYPE_PERSONNE)= ''
OR trim(A.TYPE_PERSONNE) in ('PP','MAJEUR','PDV') then trim(A.CONFORM_OCM_P_PHY_MAJEUR)
...
(CASE
WHEN trim(A.TYPE_PERSONNE) in ('MAJEUR','PDV') OR A.TYPE_PERSONNE IS NULL OR trim(A.TYPE_PERSONNE) = '' OR
...
(cast(months_between('###SLICE_VALUE###', A.date_expiration) as int) < 6) AND
A.MULTI_SIM = 'NON' AND
...
(CASE
WHEN A.TYPE_PERSONNE in ('MAJEUR','PDV') OR A.TYPE_PERSONNE IS NULL OR trim(A.TYPE_PERSONNE) = '' OR A.TYPE_PERSONNE = 'PP' THEN
...
OR A.MULTI_SIM = 'OUI'
OR (cast(months_between('###SLICE_VALUE###', A.date_expiration) as int) >= 6)
then 'NON' else 'OUI'
end)
ELSE NULL
...
(case when UPPER(trim(A.STATUT)) in (UPPER('Suspendu')) then 'OUI' else 'NON' end) as  EST_SUSPENDU,
(CASE WHEN A.NOM_STRUCTURE IS NULL OR trim(A.NOM_STRUCTURE) = '' THEN 'OUI' ELSE 'NON' END) NOM_STRUCTURE_ABSENT,
(CASE WHEN A.NUMERO_REGISTRE_COMMERCE IS NULL OR trim(A.NUMERO_REGISTRE_COMMERCE) = '' THEN 'OUI' ELSE 'NON' END) NUMERO_REGISTRE_ABSENT,
