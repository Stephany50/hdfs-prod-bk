--- Remonter des informations des tables de revenus et calcul des champs de conformité
...
DATE_NAISSANCE,
DATE_EXPIRATION,
(case when length(UPPER(trim(a.adresse))) = 0 then a.ville||"  "||a.quartier else a.adresse end) AS ADRESSE,
...
trim(A.NUMERO_PIECE_REPRESENTANT_LEGAL) AS NUMERO_PIECE_REP_LEGAL,
DATE_ACTIVATION,
DATE_CHANGEMENT_STATUT,
UPPER(trim(A.STATUT)) AS STATUT,
...
(CASE WHEN trim(A.ADRESSE ) = '' OR A.ADRESSE IS NULL THEN 'OUI' ELSE 'NON' END) ADRESSE_ABSENT,
(CASE WHEN (TRIM(TRANSLATE(LOWER(trim(A.ADRESSE)), 'abcdefghijklmnopqrstuvwxyz123456MCMLXXXIV()[]_7890çéèàäëüï\'öî¿ôûâê,-./:',' ')) is not null
AND TRIM(TRANSLATE(LOWER(trim(A.ADRESSE)), 'abcdefghijklmnopqrstuvwxyz123456MCMLXXXIV()[]_7890çéèàäëüïöî¿ôû\'âê,-./:',' ')) <> ''
)
OR (TRIM(TRANSLATE(trim(A.ADRESSE), '12345678\'90-./:',' ')) =  ''
OR TRIM(TRANSLATE(trim(A.ADRESSE), '12345678\'90-./:',' ')) is null
)
OR (TRIM(TRANSLATE(LOWER(trim(A.ADRESSE)), 'bcdfghjklmnpqrstv\'wxzç-./,:',' '))  =  ''
OR TRIM(TRANSLATE(LOWER(trim(A.ADRESSE)), 'bcdfghjklmnpqrstvw\'xzç-./,:',' ')) is null
)
OR (TRIM(TRANSLATE(LOWER(trim(A.ADRESSE)), 'aeiouéèàäëüïöîôû\'âê-./:',' '))   =  ''
OR TRIM(TRANSLATE(LOWER(trim(A.ADRESSE)), 'aeiouéèàäëüïöî\'ôûâê-./:',' ')) is null
)
OR (LENGTH(TRIM(A.ADRESSE)) < 2 AND not(TRIM(A.ADRESSE) is null or TRIM(A.ADRESSE) = ''))
OR TRIM(LOWER(trim(A.ADRESSE))) IN ('n/a', 'nan', 'unknown', 'inconnue', 'sans adresse','sans','non renseignée') THEN 'OUI' ELSE 'NON'
...
WHERE ORIGINAL_FILE_DATE =  date_add(to_date('###SLICE_VALUE###'), 1) and type_personne not in ('M2M','FLOTTE')) A
LEFT JOIN
(SELECT AA.*, BB.LONGITUDE, BB.LATITUDE FROM
(SELECT * FROM (SELECT a.*, ROW_NUMBER() OVER (PARTITION  BY msisdn ORDER BY LAST_LOCATION_DAY DESC, insert_date desc) RN
FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY a WHERE EVENT_DATE = to_date('###SLICE_VALUE###')) x WHERE RN=1) AA
LEFT JOIN(SELECT * FROM (SELECT DISTINCT SITE_NAME,LONGITUDE,LATITUDE,
ROW_NUMBER() OVER (PARTITION BY SITE_NAME ORDER BY SITE_NAME DESC) AS RANG
FROM DIM.SPARK_DT_GSM_CELL_CODE) y WHERE RANG = 1) BB ON upper(trim(AA.SITE_NAME)) = upper(trim(BB.SITE_NAME))
...
LEFT JOIN (select OFFER_PROFILE_ID, CUSTOMER_TYPE, CUSTOMER_PROFILE, OFFER_NAME, PROFILE_CODE, PROFILE_NAME, INITIAL_CREDIT, RESALE_OF_TRAFFIC, RATEPLAN_ID, PLATFORM, CRM_SEGMENTATION,
...
WHERE EVENT_MONTH IN (substr(add_months(to_date('###SLICE_VALUE###'),-1),1,7),substr(add_months(to_date('###SLICE_VALUE###'),-2),1,7),substr(add_months(to_date('###SLICE_VALUE###'),-3),1,7))
GROUP BY MSISDN) D ON FN_FORMAT_MSISDN_TO_9DIGITS(trim(A.MSISDN)) = FN_FORMAT_MSISDN_TO_9DIGITS(trim(D.MSISDN))
LEFT JOIN (SELECT MDM.MSISDN,
...
WHERE MOIS IN (substr(add_months(to_date('###SLICE_VALUE###'),-1),1,7),substr(add_months(to_date('###SLICE_VALUE###'),-2),1,7),substr(add_months(to_date('###SLICE_VALUE###'),-3),1,7)) GROUP BY MSISDN
