insert into TMP.TT_FT_BDI_AMELIORE
SELECT
...
(CASE WHEN (trim(A.MSISDN) = trim(A.NUMERO_PIECE)) or (substr(trim(A.MSISDN),-9,9) = trim(A.NUMERO_PIECE)) THEN 'OUI' ELSE 'NON' END) NUMERO_PIECE_EGALE_MSISDN,
...
FROM (
SELECT *
FROM CDR.SPARK_IT_BDI_AMELIORE
