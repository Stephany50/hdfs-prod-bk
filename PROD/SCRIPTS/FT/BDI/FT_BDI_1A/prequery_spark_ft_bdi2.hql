...
if(ft_bdi.nb_ft_bdi = 0 and it_bdi.nb_it_bdi >= 10 and ft_clsd.nb_ft_clsd >= 10 and ft_mdm.nb_ft_mdm >= 10
     and ft_dmomm.nb_ft_dmomm >= 10 and ft_bdi_prev.nb_ft_bdi >= 10
            ...
            ,'OK','NOK')
FROM (select count(*) as nb_ft_bdi from MON.SPARK_FT_BDI_1A where event_date=to_date('###SLICE_VALUE###')) ft_bdi
,(select count(*) as nb_ft_bdi from MON.SPARK_FT_BDI_1A where event_date=DATE_SUB(to_date('###SLICE_VALUE###'),1)) ft_bdi_prev
,(SELECT count(*) as nb_it_bdi FROM CDR.SPARK_IT_BDI_1A where original_file_date=date_add(to_date('###SLICE_VALUE###'),1)) it_bdi
