...
FROM MON.SPARK_FT_BDI a where event_date = '###SLICE_VALUE###' and multi_sim='OUI' and est_suspendu = 'NON' and TYPE_PERSONNE IN ('MAJEUR','PP', 'MINEUR') and statut_derogation = 'NON') agg,
(select
count(distinct trim(upper(trim(nom)||' '||trim(prenom)))) NBR_TOTAL_NOM_PRE_INVAL,
count(*) NBR_LIGNES_NOM_PRE_INVAL
from MON.SPARK_FT_BDI where event_date = '###SLICE_VALUE###' and multi_sim='OUI' and est_suspendu = 'NON' and TYPE_PERSONNE IN ('MAJEUR','PP', 'MINEUR') and statut_derogation = 'NON' and
...
FROM (select trim(upper(trim(nom)||' '||trim(prenom))) VAL_ATTRIB,count(*) COUNT_ATTRIB from MON.SPARK_FT_BDI where event_date = '###SLICE_VALUE###' and multi_sim='OUI' and est_suspendu = 'NON' and TYPE_PERSONNE IN ('MAJEUR','PP', 'MINEUR') and statut_derogation = 'NON'
...
a.COUNT_ATTRIB,current_timestamp INSERT_DATE,'###SLICE_VALUE###' EVENT_DATE
FROM (select numero_piece VAL_ATTRIB,count(*) COUNT_ATTRIB from MON.SPARK_FT_BDI where event_date = '###SLICE_VALUE###' and multi_sim='OUI' and est_suspendu = 'NON' and TYPE_PERSONNE IN ('MAJEUR','PP', 'MINEUR') and statut_derogation = 'NON'
