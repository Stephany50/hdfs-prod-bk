...
(SELECT
    ...
    sum(case when trim(date_souscription_an) = 'OUI' and B.msisdn is null and trim(statut) in ('SUSPENDU_SORTANT','SUSPENDU_ENTRANT','ACTIF') then 1 else 0 end) as date_souscription_an_new,
    ...
    sum(case when trim(nom_prenom_an) = 'OUI' and B.msisdn is null and trim(statut) in ('SUSPENDU_SORTANT','SUSPENDU_ENTRANT','ACTIF') then 1 else 0 end) as nom_prenom_an_new,
    sum(case when trim(numero_piece_an) = 'OUI' and B.msisdn is null and trim(statut) in ('SUSPENDU_SORTANT','SUSPENDU_ENTRANT','ACTIF') then 1 else 0 end) as numero_piece_an_new,
    sum(case when trim(imei_an) = 'OUI' and B.msisdn is null and trim(statut) in ('SUSPENDU_SORTANT','SUSPENDU_ENTRANT','ACTIF') then 1 else 0 end) as imei_an_new,
    sum(case when trim(adresse_an) = 'OUI' and B.msisdn is null and trim(statut) in ('SUSPENDU_SORTANT','SUSPENDU_ENTRANT','ACTIF') then 1 else 0 end) as adresse_an_new,
