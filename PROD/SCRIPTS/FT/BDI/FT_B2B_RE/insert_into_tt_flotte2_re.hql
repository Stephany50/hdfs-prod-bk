...
date_expiration, ville, quartier, statut_old, raison_statut, odbic, odboc, date_changement_statut,
...
from TMP.tt_flotte02_RE xx
left join (select distinct msisdn from TMP.tt_flotte1_RE) xy
on trim(xx.msisdn) = trim(xy.msisdn)
where xy.msisdn is null
) A
left join
(select
...
disponibilite_scan,
type_client
from TMP.tt_pm01_RE) B
on upper(trim(A.compte_client_structure)) =  upper(trim(B.compte_client_parent))