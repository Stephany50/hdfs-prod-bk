...
when trim(A.TYPE_PERSONNE) in ('FLOTTE','PERSONNE_MORALE') then trim(A.CONFORM_ART_P_MORALE_FLOTTE)
when trim(A.TYPE_PERSONNE) in ('M2M','MACHINE_2_MACHINE') then trim(A.CONFORM_ART_P_MORALE_M2M)
END) AS CONFORME_ART,
case when trim(A.TYPE_PERSONNE) is null
OR trim(A.TYPE_PERSONNE)= ''
...
when trim(A.TYPE_PERSONNE) in ('FLOTTE','PERSONNE_MORALE') then trim(A.CONFORM_OCM_P_MORALE_FLOTTE)
when trim(A.TYPE_PERSONNE) in  ('M2M','MACHINE_2_MACHINE') then trim(A.CONFORM_OCM_P_MORALE_M2M)
END AS CONFORME_OCM,
...
A.*,
(CASE
WHEN trim(A.TYPE_PERSONNE) in ('M2M','MACHINE_2_MACHINE') THEN
(
CASE WHEN (
...
(CASE
WHEN trim(A.TYPE_PERSONNE) in ('FLOTTE','PERSONNE_MORALE') THEN
(
CASE WHEN (
A.NOM_STRUCTURE IS NOT NULL AND trim(A.NOM_STRUCTURE) <> '' AND
(A.NUMERO_REGISTRE_COMMERCE IS NOT NULL AND trim(A.NUMERO_REGISTRE_COMMERCE) <> '' AND
...
(case when trim(A.ODBOUTGOINGCALLS) is null OR trim(A.ODBINCOMINGCALLS)  is null then 'U'
