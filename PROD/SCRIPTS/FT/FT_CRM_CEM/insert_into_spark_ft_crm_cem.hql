...
SELECT
(CASE WHEN upper(trim('')) IN ('M.', 'MRS', 'MR', 'MLLE', 'MME', 'MLLE.', 'MME.') THEN '' else NULL END) as CIVILITE,
D.NOM,
D.PRENOM,
ACCESS_KEY,
MAIN_IMSI,
...
FROM (SELECT * from MON.SPARK_FT_CONTRACT_SNAPSHOT where EVENT_DATE= date_add('###SLICE_VALUE###', 1)) A
...
LEFT JOIN (select msisdn, nom, prenom from CDR.SPARK_IT_KYC_BDI_FULL where original_file_date = date_add('###SLICE_VALUE###', 1)) D ON upper(trim(A.ACCESS_KEY)) = upper(trim(D.MSISDN))
LEFT JOIN (SELECT * FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE = '###SLICE_VALUE###') E ON upper(trim(A.ACCESS_KEY)) = upper(trim(E.MSISDN))
LEFT JOIN (SELECT * from MON.SPARK_FT_IMEI_ONLINE where SDATE = '###SLICE_VALUE###') F ON upper(trim(A.ACCESS_KEY)) = upper(trim(F.MSISDN))
LEFT JOIN (SELECT MSISDN, SUM(NBYTEST)/(1024*1024*1024) NBYTEST FROM MON.SPARK_FT_OTARIE_DATA_TRAFFIC_DAY WHERE TRANSACTION_DATE = '###SLICE_VALUE###' GROUP BY MSISDN) G ON upper(trim(A.ACCESS_KEY)) = upper(trim(G.MSISDN))
...
WHERE upper(trim(OSP_STATUS)) = 'ACTIVE' OR upper(trim(OSP_STATUS)) = 'INACTIVE'