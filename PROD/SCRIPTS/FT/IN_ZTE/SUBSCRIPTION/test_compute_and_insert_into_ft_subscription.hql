--INSERT INTO MON.FT_CBM_BUNDLE_SUBS_DAILY PARTITION(PERIOD)
    ...
    SUM(VOIX_ONNET) AS AMOUNT_VOICE_ONNET,
    SUM(VOIX_OFFNET)  AS AMOUNT_VOICE_OFFNET,
    SUM(VOIX_INTER)  AS AMOUNT_VOICE_INTER,
    SUM(VOIX_ROAMING)  AS AMOUNT_VOICE_ROAMING,
    SUM(SMS_ONNET)  AS AMOUNT_SMS_ONNET,
    SUM(SMS_OFFNET)  AS AMOUNT_SMS_OFFNET,
    SUM(SMS_INTER)  AS AMOUNT_SMS_INTER,
    SUM(SMS_ROAMING)  AS AMOUNT_SMS_ROAMING,
    SUM(DATA_BUNDLE) AS AMOUNT_DATA,
    SUM(SVA)  AS AMOUNT_SVA,
    BENEFIT_BAL_LIST,
    '${hivevar:date}' AS PERIOD
    ...
    FROM (
        ...
        FROM
            (SELECT
               ...
               , MAX(CASE WHEN  ITSUBSC.CHANNEL_ID='32' THEN NVL(AMOUNT_VIA_OM_VAS,0)
                     ELSE EVENT_COST / 100
                     END )    RATED_AMOUNT
             ...
             (
               ...
               (
                 ...
                 (
                     ...
                     WHERE A.CREATEDDATE = '${hivevar:date}'
                 ) A
             ...
             LEFT JOIN (SELECT NVL(PRICE_PLAN_CODE, 'UNKNOWN')PRICE_PLAN_CODE , MIN( PRICE_PLAN_NAME) PRICE_PLAN_NAME
                        ...
                        WHERE ORIGINAL_FILE_DATE = '${hivevar:date}'
