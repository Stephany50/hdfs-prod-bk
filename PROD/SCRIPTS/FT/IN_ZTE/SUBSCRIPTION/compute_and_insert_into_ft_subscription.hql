...
FROM
    (SELECT
       ...
       , MAX(CASE WHEN  ITSUBSC.CHANNEL_ID='32' THEN NVL(AMOUNT_VIA_OM,0)
             ELSE EVENT_COST / 100
             END )    RATED_AMOUNT
     ...
     (
       ...
       (
         ...
         (
             ...
             FROM CDR.IT_ZTE_SUBSCRIPTION A
     ...
     LEFT JOIN (SELECT
                    MAX(IPP_AMOUNT) AMOUNT_VIA_OM,
                    IPP_NAME
                    FROM CDR.IT_ZTE_IPP_EXTRACT IPP_EXTRACT
                    WHERE IPP_EXTRACT.ORIGINAL_FILE_DATE IN (SELECT MAX(ORIGINAL_FILE_DATE) ORIGINAL_FILE_DATE FROM CDR.IT_ZTE_IPP_EXTRACT)
