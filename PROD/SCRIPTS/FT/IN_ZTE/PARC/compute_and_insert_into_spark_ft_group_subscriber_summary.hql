INSERT INTO MON.SPARK_FT_GROUP_SUBSCRIBER_SUMMARY PARTITION(EVENT_DATE)
SELECT
  PROFILE
  , STATUT
  , ETAT
...
FROM (
  ...
  GROUP BY
      ...
      , a.SRC_TABLE, NVL (a.OSP_CONTRACT_TYPE, 'PURE PREPAID' ), a.OSP_STATUS,a.location_ci
  ...
  GROUP BY
      ...
      , b.SRC_TABLE, NVL (UPPER(d.CONTRACT_TYPE), 'PURE PREPAID' ), b.PLATFORM_STATUS,location_ci
  ...
  FROM MON.SPARK_FT_CONTRACT_SNAPSHOT a
  LEFT JOIN (
    SELECT * FROM MON.SPARK_FT_ACCOUNT_ACTIVITY  WHERE EVENT_DATE = '###SLICE_VALUE###'
   ) b ON a.ACCESS_KEY = b.MSISDN
  WHERE -- ROWNUM < 1 AND
      a.EVENT_DATE = '###SLICE_VALUE###'
