INSERT INTO MON.FT_COMMERCIAL_SUBSCRIB_SUMMARY PARTITION(DATECODE)
...
FROM
    (
        ...
        LEFT JOIN (SELECT  MSISDN, COMGP_STATUS_DATE, COMGP_STATUS, COMGP_FIRST_ACTIVE_DATE
                  ...
                  FROM MON.FT_ACCOUNT_ACTIVITY   -- ROWNUM < 1 AND
...
FROM (
  ...
  FROM (
      ...
      FROM (
        SELECT
          a.MSISDN
        FROM MON.FT_ACCOUNT_ACTIVITY a
  ...
  LEFT JOIN (
    ...
    FROM MON.FT_ACCOUNT_ACTIVITY  WHERE EVENT_DATE = '###SLICE_VALUE###'
  ) b ON a.ACCESS_KEY = b.MSISDN
