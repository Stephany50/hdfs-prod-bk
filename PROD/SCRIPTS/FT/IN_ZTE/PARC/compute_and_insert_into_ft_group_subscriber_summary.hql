...
FROM (
    ...
    , NVL (b.GP_STATUS, 'INACT') STATUT
     ...
     , ROUND (
          CASE
              ...
              WHEN  DATEDIFF('2019-06-18', a.ACTIVATION_DATE)-1  < 0 THEN 0
              WHEN  DATEDIFF('2019-06-18', a.ACTIVATION_DATE)-1 > 365
                      THEN ROUND  ((DATEDIFF('2019-06-18',a.ACTIVATION_DATE )-1)/(12 * 30.4375) )*12
              ELSE (DATEDIFF('2019-06-18',a.ACTIVATION_DATE )-1)/30.4375
          END
      ) TRANCHE_AGE
     ...
     , SUM (1) EFFECTIF
     , SUM (IF(a.ACTIVATION_DATE=DATE_SUB('2019-06-18',1),1,0)) ACTIVATIONS
     , SUM (IF(a.DEACTIVATION_DATE=DATE_SUB('2019-06-18',1),1,0)) RESILIATIONS
     , a.SRC_TABLE
  ...
  LEFT JOIN (
    SELECT * FROM JUNK.FT_ACCOUNT_ACTIVITY
    WHERE
    EVENT_DATE = '2019-06-18'
  ) b ON a.ACCESS_KEY = b.MSISDN
  WHERE
      a.EVENT_DATE = '2019-06-18'
  ...
  SELECT
    ...
    , ROUND (
        CASE
            WHEN (b.ACTIVATION_DATE IS NULL) OR (b.ACTIVATION_DATE < TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP('19880101','yyyyMMdd')))) THEN 0
            WHEN  DATEDIFF('2019-06-18',b.ACTIVATION_DATE )-1 < 0 THEN 0
            WHEN  DATEDIFF('2019-06-18',b.ACTIVATION_DATE )-1 > 365
                    THEN ROUND  ((DATEDIFF('2019-06-18',b.ACTIVATION_DATE )-1)/(12 * 30.4375) )*12
            ELSE (DATEDIFF('2019-06-18',b.ACTIVATION_DATE )-1)/30.4375
        END
    ) TRANCHE_AGE
     , 0 CREDIT
     , SUM (1) EFFECTIF
     , SUM (IF(b.ACTIVATION_DATE=DATE_SUB('2019-06-18',1),1,0)) ACTIVATIONS
     , SUM (IF(b.RESILIATION_DATE=DATE_SUB('2019-06-18',1),1,0)) RESILIATIONS
     , b.SRC_TABLE
     , CURRENT_TIMESTAMP INSERT_DATE
     , NVL (UPPER(d.CONTRACT_TYPE), 'PURE PREPAID' ) CUST_BILLCYCLE
     , b.PLATFORM_STATUS PLATFORM_STATUS
     , '2019-06-18' EVENT_DATE
  FROM JUNK.FT_ACCOUNT_ACTIVITY b
  LEFT JOIN (
    SELECT
      a.MSISDN
    FROM JUNK.FT_ACCOUNT_ACTIVITY a
    LEFT JOIN (SELECT access_key MSISDN FROM JUNK.FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = '2019-06-18') b ON a.MSISDN=b.MSISDN
    WHERE a.EVENT_DATE = '2019-06-18' AND NVL (GP_STATUS, 'INACT')='ACTIF' AND b.MSISDN IS NULL
  ) c ON b.MSISDN = c.MSISDN
  LEFT JOIN DIM.DT_OFFER_PROFILES d ON b.FORMULE=d.PROFILE_CODE
  WHERE b.EVENT_DATE = '2019-06-18'
  ...
  SELECT
   ...
   , SUM ((CASE WHEN MAIN_CREDIT IS NULL THEN 0 WHEN MAIN_CREDIT < 0 THEN 0 ELSE MAIN_CREDIT END )
          + (CASE WHEN PROMO_CREDIT IS NULL THEN 0 WHEN PROMO_CREDIT < 0 THEN 0 ELSE PROMO_CREDIT END ) ) CREDIT
   , SUM (1) EFFECTIF
   , SUM (IF(a.BSCS_ACTIVATION_DATE=DATE_SUB('2019-06-18',1),1,0)) ACTIVATIONS
   , SUM (IF(a.BSCS_DEACTIVATION_DATE=DATE_SUB('2019-06-18',1),1,0)) RESILIATIONS
  ...
  FROM X a
  LEFT JOIN (
    SELECT * FROM JUNK.FT_ACCOUNT_ACTIVITY  WHERE EVENT_DATE = '2019-06-18'
   ) b ON a.ACCESS_KEY = b.MSISDN
  WHERE -- ROWNUM < 1 AND
      a.EVENT_DATE = '2019-06-18'
