...
LEFT JOIN (SELECT * FROM MON.FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = '###SLICE_VALUE###' AND OSP_STATUS <> 'TERMINATED') B ON REGEXP_REPLACE(A.SENDER_MSISDN, "^237+(?!$)", "") = B.ACCESS_KEY
LEFT JOIN (SELECT * FROM MON.FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = '###SLICE_VALUE###' AND OSP_STATUS <> 'TERMINATED') C ON REGEXP_REPLACE(A.RECEIVER_MSISDN, "^237+(?!$)", "") = C.ACCESS_KEY;
