...
LEFT JOIN (SELECT COUNT(DISTINCT SENDER_MSISDN) COUNT_SENDER_MSISDN FROM MON.SPARK_FT_DATA_TRANSFER WHERE TRANSACTION_DATE = '###SLICE_VALUE###') TMP1
LEFT JOIN (SELECT COUNT(DISTINCT RECEIVER_MSISDN) COUNT_RECEIVER_MSISDN FROM MON.SPARK_FT_DATA_TRANSFER WHERE TRANSACTION_DATE = '###SLICE_VALUE###') TMP2
LEFT JOIN (
SELECT COUNT(DISTINCT MSISDN) COUNT_DISTINCT_DAILY_USERS FROM
    (
    SELECT RECEIVER_MSISDN MSISDN FROM MON.SPARK_FT_DATA_TRANSFER WHERE TRANSACTION_DATE = '###SLICE_VALUE###'
    UNION ALL
    SELECT SENDER_MSISDN MSISDN FROM MON.SPARK_FT_DATA_TRANSFER WHERE TRANSACTION_DATE = '###SLICE_VALUE###'
    ) T
) TMP3
GROUP BY 
    A.TRANSACTION_DATE,     
