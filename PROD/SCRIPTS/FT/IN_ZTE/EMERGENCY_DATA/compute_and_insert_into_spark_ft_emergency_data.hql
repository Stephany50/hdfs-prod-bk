INSERT INTO MON.SPARK_FT_EMERGENCY_DATA PARTITION(TRANSACTION_DATE)
SELECT
    MSISDN,
    TRANSACTION_TIME,
    AMOUNT,
 ...
 FROM (
    ...
    FROM
     (
        ...
        FROM CDR.SPARK_IT_ZTE_EMERGENCY_DATA a
        ...
        LEFT JOIN (SELECT * FROM MON.SPARK_FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = '###SLICE_VALUE###' AND OSP_STATUS <> 'TERMINATED') b ON IF(regexp_replace( a.MSISDN,"^(237)+(?!$)","")='237',NULL,regexp_replace( a.MSISDN,"^(237)+(?!$)",""))= b.ACCESS_KEY
