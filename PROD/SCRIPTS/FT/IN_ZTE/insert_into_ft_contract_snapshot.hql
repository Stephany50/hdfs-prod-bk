...
(
  ...
  (
    SELECT
      ...
      LEFT JOIN CDR.IT_ZTE_CUST_EXTRACT CUST ON SUBS.SUBS_CUST_ID = CUST.CUST_ID
      LEFT JOIN CDR.IT_ZTE_PROD_EXTRACT PROD ON SUBS.SUBS_ID = PROD.PROD_ID
      LEFT JOIN CDR.IT_ZTE_PRICE_PLAN_EXTRACT PRICE_PLAN ON SUBS.PRICE_PLAN_ID = PRICE_PLAN.PRICE_PLAN_ID
      LEFT JOIN CDR.IT_ZTE_ACCT_EXTRACT ACCT ON SUBS.SUBS_ACCT_ID = ACCT.ACCT_ID
      LEFT JOIN (
        SELECT ACCT_ID
          ,NULL MAIN_BALANCE
          ,NULL PROMO_BALANCE
    ...
    WHERE
          ACC_NBR.ORIGINAL_FILE_DATE = '###SLICE_VALUE###'
      AND CUST.ORIGINAL_FILE_DATE = '###SLICE_VALUE###'
      AND PROD.ORIGINAL_FILE_DATE = '###SLICE_VALUE###'
      AND ACCT.ORIGINAL_FILE_DATE = '###SLICE_VALUE###'
      AND PRICE_PLAN.ORIGINAL_FILE_DATE = '###SLICE_VALUE###'
      AND PROD.INDEP_PROD_ID IS NULL
      AND NVL(PROD.ROUTING_ID, '100') IN ('1','2','3')
      AND ACC_NBR.ACC_NBR_STATE = 'C' --USED
      AND PROD.PROD_STATE <> 'B'
  ) CUR
