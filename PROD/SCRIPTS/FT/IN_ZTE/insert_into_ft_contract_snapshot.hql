...
(
  ...
  (
  SELECT 
    TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP('2019-09-19', 'yyyy-MM-dd'))) EVENT_DATE
    -- ACC_NBR Source
    ,ACC_NBR.ACC_NBR
  ...
  FROM ( SELECT ACC_NBR, STATE_DATE, ACC_NBR_STATE, ACC_NBR_TYPE_ID FROM CDR.IT_ZTE_ACC_NBR_EXTRACT WHERE ORIGINAL_FILE_DATE = '2019-09-19' AND ACC_NBR_STATE = 'C') ACC_NBR
  LEFT JOIN (SELECT ACC_NBR SUBS_ACC_NBR, IMSI, UPDATE_DATE, CUST_ID SUBS_CUST_ID, ACCT_ID SUBS_ACCT_ID, PRICE_PLAN_ID, DEF_LANG_ID, SUBS_ID  FROM CDR.IT_ZTE_SUBS_EXTRACT WHERE ORIGINAL_FILE_DATE = '2019-09-19') SUBS  ON ACC_NBR.ACC_NBR = SUBS.SUBS_ACC_NBR
  LEFT JOIN (SELECT CUST_ID, CUST_CODE, CUST_NAME, ADDRESS, CUST_TYPE, PARENT_ID, CREATED_DATE, UPDATE_DATE, STATE, STATE_DATE FROM CDR.IT_ZTE_CUST_EXTRACT WHERE ORIGINAL_FILE_DATE = '2019-09-19') CUST ON SUBS.SUBS_CUST_ID = CUST.CUST_ID
  LEFT JOIN (SELECT PROD_ID, CREATED_DATE, BLOCK_REASON, COMPLETED_DATE, STATE, PROD_STATE, PROD_STATE_DATE, UPDATE_DATE, STATE_DATE, SP_ID FROM CDR.IT_ZTE_PROD_EXTRACT WHERE ORIGINAL_FILE_DATE = '2019-09-19' AND INDEP_PROD_ID IS NULL AND NVL(ROUTING_ID, '100') IN ('1','2','3') AND PROD_STATE <> 'B') PROD ON SUBS.SUBS_ID = PROD.PROD_ID
  ...
  LEFT JOIN (SELECT ACCT_ID, UPDATE_DATE, CREATED_DATE FROM CDR.IT_ZTE_ACCT_EXTRACT WHERE ORIGINAL_FILE_DATE = '2019-09-19') ACCT ON SUBS.SUBS_ACCT_ID = ACCT.ACCT_ID
  LEFT JOIN (
  SELECT ACCT_ID
  ,NULL MAIN_BALANCE
  ,NULL PROMO_BALANCE
  ...
  WHERE BAL.ORIGINAL_FILE_DATE = '2019-09-19'
  GROUP BY ACCT_ID
  ) BAL ON SUBS.SUBS_ACCT_ID = BAL.ACCT_ID
  ) CUR
  LEFT JOIN (SELECT DISTINCT ACCESS_KEY, STATE_DATETIME, CURRENT_STATUS_1 CURRENT_STATUS, PREVIOUS_STATUS FROM MON.FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = DATE_SUB(TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP('2019-09-19', 'yyyy-MM-dd'))), 1)) PREV ON (CUR.ACC_NBR = PREV.ACCESS_KEY AND CUR.PROD_CREATED_DATE = PREV.STATE_DATETIME)
) ZTE
