...
(
  ...
  LEFT JOIN (SELECT PRICE_PLAN_ID, PRICE_PLAN_NAME FROM CDR.SPARK_IT_ZTE_PRICE_PLAN_EXTRACT WHERE ORIGINAL_FILE_DATE in (select max(original_file_date) from cdr.IT_ZTE_PRICE_PLAN_EXTRACT)) PRICE_PLAN ON SUBS.PRICE_PLAN_ID = PRICE_PLAN.PRICE_PLAN_ID
