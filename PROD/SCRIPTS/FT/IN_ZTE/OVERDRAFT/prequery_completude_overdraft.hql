...
FROM
(SELECT COUNT(DISTINCT ORIGINAL_FILE_NAME) CHECK_FILE_COUNT FROM CDR.IT_ZTE_CHECK_FILE WHERE CDR_DATE = '###SLICE_VALUE###' AND WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' AND ORIGINAL_FILE_NAME NOT LIKE '%POST%')) T_4,
...
(SELECT COUNT(C.FILE_NAME) MISSING_FILES
 ...
 (
 SELECT 1 FROM
 (SELECT ORIGINAL_FILE_NAME FROM CDR.IT_ZTE_RECHARGE WHERE pay_date BETWEEN DATE_SUB('###SLICE_VALUE###',${hivevar:date_offset}) AND '###SLICE_VALUE###' AND TO_DATE(ORIGINAL_FILE_DATE) = '###SLICE_VALUE###'
UNION SELECT ORIGINAL_FILE_NAME FROM CDR.IT_ZTE_ADJUSTMENT WHERE create_date BETWEEN DATE_SUB('###SLICE_VALUE###',${hivevar:date_offset}) AND '###SLICE_VALUE###' AND TO_DATE(ORIGINAL_FILE_DATE) = '###SLICE_VALUE###'
UNION SELECT ORIGINAL_FILE_NAME FROM CDR.IT_ZTE_TRANSFER WHERE pay_date BETWEEN DATE_SUB('###SLICE_VALUE###',${hivevar:date_offset}) AND '###SLICE_VALUE###' AND TO_DATE(ORIGINAL_FILE_DATE) = '###SLICE_VALUE###'  ) B
