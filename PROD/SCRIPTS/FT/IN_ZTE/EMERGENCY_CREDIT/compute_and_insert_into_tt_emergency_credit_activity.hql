...
FROM (
        SELECT * FROM MON.FT_LAST_UPDATE_EC_EXTRACT where EVENT_DATE = '###SLICE_VALUE###'

) a
LEFT JOIN (SELECT * FROM MON.FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = DATE_SUB('###SLICE_VALUE###',-1)) b ON a.MSISDN = b.ACCESS_KEY
LEFT JOIN (
