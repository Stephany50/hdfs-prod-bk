INSERT INTO MON.SPARK_FT_CREDIT_TRANSFER PARTITION(REFILL_DATE)
SELECT
 TRANSFER_ID
 ,REFILL_TIME
 ,SENDER_MSISDN
...
LEFT JOIN ( 
  ...
  WHERE ORIGINAL_FILE_DATE in (select max(ORIGINAL_FILE_DATE) ORIGINAL_FILE_DATE FROM CDR.SPARK_IT_ZTE_PROFILE where ORIGINAL_FILE_DATE>= DATE_SUB('###SLICE_VALUE###', 7))
