INSERT INTO MON.SPARK_FT_CONTRACT_SNAPSHOT PARTITION(EVENT_DATE)
--INSERT INTO tmp.tt_contract_snap
SELECT
    ...
    -- ,(CASE WHEN UPPER(nvl(B.CONTRACT_TYPE, B.CUSTOMER_TYPE)) LIKE '%POSTPAID%' THEN 'POSTPAID' WHEN UPPER(nvl(B.CONTRACT_TYPE, B.CUSTOMER_TYPE)) LIKE '%PREPAID%' or UPPER(nvl(B.CONTRACT_TYPE, B.CUSTOMER_TYPE)) LIKE '%HYBRID%' THEN 'PREPAID' END) OSP_ACCOUNT_TYPE
    , (CASE WHEN UPPER(ZTE.SUBSCRIBER_TYPE) LIKE '%POSTPAID%' THEN 'POSTPAID' WHEN UPPER(ZTE.SUBSCRIBER_TYPE) LIKE '%PREPAID%' or UPPER(ZTE.SUBSCRIBER_TYPE) LIKE '%HYBRID%' THEN 'PREPAID' ELSE NULL END) OSP_ACCOUNT_TYPE
    ...
    ,NULL CUST_BILLCYCLE
    ,NULL CUST_SEGMENT
    -- ,UPPER(nvl(B.CONTRACT_TYPE, B.CUSTOMER_TYPE)) OSP_CONTRACT_TYPE
    -- ,UPPER(ZTE.SUBSCRIBER_TYPE) OSP_CONTRACT_TYPE
    ,(CASE WHEN UPPER(ZTE.SUBSCRIBER_TYPE) LIKE '%POSTPAID%' THEN 'PURE POSTPAID' WHEN UPPER(ZTE.SUBSCRIBER_TYPE) LIKE '%PREPAID%' THEN 'PURE PREPAID' WHEN UPPER(ZTE.SUBSCRIBER_TYPE) LIKE '%HYBRID%' THEN 'HYBRID' ELSE NULL END) OSP_CONTRACT_TYPE
...
(
  SELECT DISTINCT
    ...
    ,FIRST_VALUE(SUBSCRIBER_TYPE) OVER (PARTITION BY ACC_NBR ORDER BY PROD_CREATED_DATE DESC) SUBSCRIBER_TYPE
  ...
  FROM ( SELECT ACC_NBR, STATE_DATE, ACC_NBR_STATE, ACC_NBR_TYPE_ID FROM CDR.SPARK_IT_ZTE_ACC_NBR_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###') ACC_NBR
  LEFT JOIN (SELECT ACC_NBR SUBS_ACC_NBR, IMSI, UPDATE_DATE, CUST_ID SUBS_CUST_ID, ACCT_ID SUBS_ACCT_ID, PRICE_PLAN_ID, SUBSCRIBER_TYPE , DEF_LANG_ID, SUBS_ID, ROW_NUMBER() OVER(PARTITION BY SUBS_ID ORDER BY IMSI DESC) RN_SUBS  FROM CDR.SPARK_IT_ZTE_SUBS_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###') SUBS  ON RN_SUBS=1 AND ACC_NBR.ACC_NBR = SUBS.SUBS_ACC_NBR
  ...
  LEFT JOIN (SELECT PRICE_PLAN_ID, PRICE_PLAN_NAME FROM CDR.SPARK_IT_ZTE_PRICE_PLAN_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' ) PRICE_PLAN ON SUBS.PRICE_PLAN_ID = PRICE_PLAN.PRICE_PLAN_ID -- in (select max(original_file_date) from cdr.SPARK_IT_ZTE_PRICE_PLAN_EXTRACT)
  LEFT JOIN (SELECT ACCT_ID, UPDATE_DATE, CREATED_DATE FROM CDR.SPARK_IT_ZTE_ACCT_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###') ACCT ON SUBS.SUBS_ACCT_ID = ACCT.ACCT_ID
  LEFT JOIN (
  SELECT ACCT_ID
  ,NULL MAIN_BALANCE
  ,NULL PROMO_BALANCE
  ...
  FROM (SELECT bal.*, row_number() over (partition by bal.acct_id, bal.acct_res_id order by bal.update_date desc) rn FROM CDR.SPARK_IT_ZTE_BAL_EXTRACT bal WHERE BAL.ORIGINAL_FILE_DATE = '###SLICE_VALUE###') BAL
  ...
  LEFT JOIN (SELECT DISTINCT ACCESS_KEY, STATE_DATETIME, CURRENT_STATUS_1 CURRENT_STATUS, PREVIOUS_STATUS FROM MON.SPARK_FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = DATE_SUB('###SLICE_VALUE###', 1)) PREV ON (CUR.ACC_NBR = PREV.ACCESS_KEY AND CUR.PROD_CREATED_DATE = PREV.STATE_DATETIME)
) ZTE
LEFT JOIN DIM.SPARK_DT_OFFER_PROFILES B ON ZTE.PRICE_PLAN_NAME = B.PROFILE_CODE
LEFT JOIN (
