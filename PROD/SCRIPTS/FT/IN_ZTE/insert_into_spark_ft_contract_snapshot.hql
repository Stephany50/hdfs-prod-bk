...
(
  ...
  LEFT JOIN (SELECT ACC_NBR SUBS_ACC_NBR, IMSI, UPDATE_DATE, CUST_ID SUBS_CUST_ID, ACCT_ID SUBS_ACCT_ID, PRICE_PLAN_ID, DEF_LANG_ID, SUBS_ID  FROM CDR.SPARK_IT_ZTE_SUBS_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###') SUBS  ON ACC_NBR.ACC_NBR = SUBS.SUBS_ACC_NBR
  ...
  LEFT JOIN (SELECT DISTINCT ACCESS_KEY, STATE_DATETIME, CURRENT_STATUS_1 CURRENT_STATUS, PREVIOUS_STATUS FROM MON.SPARK_FT_CONTRACT_SNAPSHOT WHERE EVENT_DATE = DATE_SUB(TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP('###SLICE_VALUE###', 'yyyy-MM-dd'))), 1)) PREV ON (CUR.ACC_NBR = PREV.ACCESS_KEY AND CUR.PROD_CREATED_DATE = PREV.STATE_DATETIME)
) ZTE
