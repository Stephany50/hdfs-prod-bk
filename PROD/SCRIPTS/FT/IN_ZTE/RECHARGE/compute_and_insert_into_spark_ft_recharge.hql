INSERT INTO MON.SPARK_FT_RECHARGE PARTITION(TRANSACTION_DATE)
    ...
    LEFT JOIN (SELECT ACCESS_KEY,
                EVENT_DATE,
                MIN(UPPER (NVL (PROFILE, SUBSTR(NVL(BSCS_COMM_OFFER,COMMERCIAL_OFFER), INSTR(NVL(BSCS_COMM_OFFER,COMMERCIAL_OFFER),'|')+1)))) PROFILE
               FROM  MON.SPARK_FT_CONTRACT_SNAPSHOT  where EVENT_DATE in (SELECT MAX(EVENT_DATE) MAX_DATE FROM MON.SPARK_FT_CONTRACT_SNAPSHOT where  EVENT_DATE between date_sub('###SLICE_VALUE###',7) AND '###SLICE_VALUE###')
