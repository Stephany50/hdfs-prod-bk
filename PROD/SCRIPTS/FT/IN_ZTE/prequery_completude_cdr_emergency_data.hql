...
(SELECT COUNT(*) CHECK_FILE_EXIST FROM CDR.SPARK_IT_ZTE_CHECK_FILE WHERE CDR_DATE = '###SLICE_VALUE###' AND CDR_TYPE = 'ZTE_ED_CDR') T_2,
(SELECT COUNT(*) CHECK_FILE_ALL_EXIST FROM CDR.SPARK_IT_ZTE_CHECK_FILE_ALL WHERE FILE_DATE = '###SLICE_VALUE###') T_3,
(SELECT COUNT(*) CHECK_FILE_ALL_EXIST_AND_TYPE FROM CDR.SPARK_IT_ZTE_CHECK_FILE_ALL WHERE FILE_DATE = '###SLICE_VALUE###' AND FILE_TYPE = 'ZTE_ED_CDR') T_6,
(
SELECT COUNT(C.FILE_NAME) MISSING_FILES
 ...
 WHERE
   NOT EXISTS
   (
      SELECT 1  FROM CDR.SPARK_IT_ZTE_EMERGENCY_DATA B
      WHERE
         TRANSACTION_DATE BETWEEN DATE_SUB('###SLICE_VALUE###',3) AND DATE_ADD('###SLICE_VALUE###', 1)
