INSERT INTO TABLE MON.SPARK_FT_CRA_GPRS_POST PARTITION(SESSION_DATE)
SELECT
    TRANSACTION_ID,
    CALL_TYPE,
    SERVED_PARTY_MSISDN,
...
FROM (
  SELECT a.*
    ...
    FROM CDR.SPARK_IT_ZTE_DATA_POST a WHERE a.START_DATE = '###SLICE_VALUE###'
) a
   LEFT JOIN DIM.DT_RATING_EVENT re ON (a.RE_ID = re.RATING_EVENT_ID)
   ...
   LEFT JOIN (SELECT PRICE_PLAN_ID, PRICE_PLAN_NAME, PRICE_PLAN_CODE FROM CDR.IT_ZTE_PRICE_PLAN_EXTRACT WHERE ORIGINAL_FILE_DATE = '###SLICE_VALUE###' ) pp1 ON (a.PRICE_PLAN_CODE = pp1.PRICE_PLAN_CODE)
   LEFT JOIN (SELECT PROFILE_ID, MAX(upper(PROFILE_NAME)) PROFILE_NAME FROM CDR.IT_ZTE_PROFILE GROUP BY PROFILE_ID) pc1 ON(a.PROD_SPEC_ID = pc1.PROFILE_ID)
