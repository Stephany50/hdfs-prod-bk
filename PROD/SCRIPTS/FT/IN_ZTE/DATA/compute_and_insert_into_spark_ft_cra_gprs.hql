...
FROM (
    ...
    FROM (
        ...
        LEFT JOIN (SELECT PROFILE_ID, MAX(upper(PROFILE_NAME)) PROFILE_NAME FROM CDR.SPARK_IT_ZTE_PROFILE WHERE ORIGINAL_FILE_DATE='###SLICE_VALUE###' GROUP BY PROFILE_ID) pc1 ON(a.PROD_SPEC_ID = pc1.PROFILE_ID)
