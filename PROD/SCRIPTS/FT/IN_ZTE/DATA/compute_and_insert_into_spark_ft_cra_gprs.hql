INSERT INTO TABLE MON.SPARK_FT_CRA_GPRS PARTITION(SESSION_DATE)
SELECT
    TRANSACTION_ID,
    CALL_TYPE,
    SERVED_PARTY_MSISDN,
...
FROM (
    ...
    FROM (
        ...
        LEFT JOIN (SELECT PROFILE_ID, MAX(upper(PROFILE_NAME)) PROFILE_NAME FROM CDR.SPARK_IT_ZTE_PROFILE WHERE ORIGINAL_FILE_DATE IN ( select max(ORIGINAL_FILE_DATE) from CDR.SPARK_IT_ZTE_PROFILE ) GROUP BY PROFILE_ID) pc1 ON(a.PROD_SPEC_ID = pc1.PROFILE_ID)
