INSERT INTO TABLE MON.SPARK_FT_CRA_GPRS_WITH_BAL_ID PARTITION(SESSION_DATE)
SELECT
    TRANSACTION_ID,
    CALL_TYPE,
    SERVED_PARTY_MSISDN,
    ...
    VLR_NUMBER,
    USED_BAL_ID_LIST,
    USED_ACCT_RES_ID_LIST,
    USED_ACCT_ITEM_TYPE_ID_LIST,
    SESSION_DATE
FROM (
    SELECT
        ...
        MAX(CONCAT_WS('|'
        , NVL(a.ACCT_ITEM_TYPE_ID1,'')
        , NVL(a.ACCT_ITEM_TYPE_ID2,'')
        , NVL(a.ACCT_ITEM_TYPE_ID3,'')
        , NVL(a.ACCT_ITEM_TYPE_ID4,'')
        )
        ) USED_ACCT_ITEM_TYPE_ID_LIST,
        MAX(CONCAT_WS('|'
    ...
    FROM (
        ...
        FROM CDR.SPARK_IT_ZTE_DATA a WHERE a.START_DATE = '###SLICE_VALUE###'
        ) a
        ...
        LEFT JOIN (SELECT profile_id,profile_name from AGG.SPARK_ZTE_PROFILE WHERE MAX_ORIGINAL_FILE_DATE = '###SLICE_VALUE###' group by profile_id,profile_name) pc1 ON(a.PROD_SPEC_ID = pc1.PROFILE_ID)
