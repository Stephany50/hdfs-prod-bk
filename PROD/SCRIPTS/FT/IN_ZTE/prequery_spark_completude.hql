SELECT IF((T_1.CHECK_FILE_COUNT >= 24 AND  T_3.CHECK_FILE_ALL_EXIST > 0 AND (T_2.CHECK_FILE_EXIST > 0 OR T_6.CHECK_FILE_ALL_EXIST_AND_TYPE > 0)) AND T_4.MISSING_FILES = 0, 'OK','NOK')
FROM
(SELECT COUNT(DISTINCT ORIGINAL_FILE_NAME) CHECK_FILE_COUNT FROM CDR.SPARK_IT_ZTE_CHECK_FILE WHERE FILE_DATE = '###SLICE_VALUE###' AND ORIGINAL_FILE_NAME LIKE '%${hivevar:check_file_regex}%') T_1,
