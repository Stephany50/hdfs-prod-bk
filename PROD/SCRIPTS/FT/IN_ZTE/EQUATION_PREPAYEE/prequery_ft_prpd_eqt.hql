SELECT IF(FT_NOT_EXISTS = 0 and FT_A_EXISTS>0 and SUBS_EXTRACT_NEXT_EXISTS>0  and PROD_EXTRACT_NEXT_EXISTS>0
    and  PRICE_PLAN_EXTRACT_NEXT_EXISTS>0 and BAL_SNAP_EXISTS>0 and BAL_SNAP_PREV_EXISTS>0 ,"OK","NOK") TABLES_EXIST
FROM
...
(SELECT COUNT(*) BAL_SNAP_EXISTS FROM CDR.IT_ZTE_BAL_SNAP WHERE ORIGINAL_FILE_DATE='###SLICE_VALUE###') I,
(SELECT COUNT(*) BAL_SNAP_PREV_EXISTS FROM CDR.IT_ZTE_BAL_SNAP WHERE ORIGINAL_FILE_DATE=DATE_SUB('###SLICE_VALUE###',1)) J;