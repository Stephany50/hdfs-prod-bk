SELECT IF( (A.CHECK_FILE_COUNT >=24 and B.CHECK_FILE_ALL_EXIST > 0) AND T1.MISSING_DATA = 0 AND T3.MISSING_VOICESMS = 0 AND T5.MISSING_ADJUSTMENT = 0 AND T6.MISSING_BALRESET = 0
    AND T7.MISSING_RECHARGE = 0 AND T8.MISSING_TRANSFER = 0 AND T9.MISSING_SUBSCRIPTION = 0 AND T10.MISSING_ED = 0 AND T11.MISSING_EC= 0 AND T12.MISSING_RECURRING=0 , 'OK','NOK') COMPLETUDE
FROM
