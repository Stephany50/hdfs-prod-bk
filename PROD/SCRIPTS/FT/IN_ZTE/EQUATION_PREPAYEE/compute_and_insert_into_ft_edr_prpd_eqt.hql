...
-------------- SOUSCRIPTIONS -----------
INSERT INTO TABLE MON.FT_EDR_PRPD_EQT PARTITION(EVENT_DATE)
SELECT NQ_CREATEDDATE EVENT_TIME, SUBSTRING(ACC_NBR, -9) MSISDN, EVENT_COST/100 MAIN_DEBIT, 0 LOAN_DEBIT, 0 SASSAYE_DEBIT,
...
FROM (
    ...
    FROM ( SELECT * FROM CDR.IT_ZTE_SUBSCRIPTION WHERE CREATEDDATE = '2019-03-01' ) A
...
------------- ADJUSTMENT ------------
INSERT INTO TABLE MON.FT_EDR_PRPD_EQT PARTITION(EVENT_DATE)
SELECT FROM_UNIXTIME(unix_timestamp(NQ_CREATE_DATE), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, 4, 9)  MSISDN,
...
-------------- TRANSFER ----------------
INSERT INTO TABLE MON.FT_EDR_PRPD_EQT PARTITION(EVENT_DATE)
SELECT FROM_UNIXTIME(unix_timestamp(PAY_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR_OUT, 4, 9) MSISDN,
    CASE WHEN ACCT_RES_CODE = 1 AND CHARGE>0 THEN CHARGE ELSE 0 END/100 MAIN_DEBIT,
    CASE WHEN ACCT_RES_CODE = 20 AND CHARGE>0 THEN CHARGE ELSE 0 END/100 LOAN_DEBIT,
    CASE WHEN ACCT_RES_CODE = 21 AND CHARGE>0 THEN CHARGE ELSE 0 END/100 SASSAYE_DEBIT,
    0 MAIN_CREDIT, 0 LOAN_CREDIT, 0 SASSAYE_CREDIT,
...
GROUP BY PAY_TIME, PAY_DATE, TRANSACTIONSN, ACC_NBR_OUT, ACCT_RES_CODE, CHANNEL_ID, CHARGE, ADD_FEE, LOAN_AMOUNT
    ;
        ...
        ------------- Recepteur du transfert------------------------
INSERT INTO TABLE MON.FT_EDR_PRPD_EQT PARTITION(EVENT_DATE)
SELECT FROM_UNIXTIME(unix_timestamp(PAY_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR_IN, 4, 9) MSISDN,
...
WHERE TRANSACTION_DATE  = '2019-03-12'
