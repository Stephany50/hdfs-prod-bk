...
SELECT FROM_UNIXTIME(unix_timestamp(PAY_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9) MSISDN,
    ...
    CASE WHEN CHANNEL_ID=9 THEN 'RECHARGE SOS DATA ' else 'RECHARGE ' end ||upper(USAGE_DESCRIPTION)||' '||a.channel_id TYPE, current_timestamp() INSERT_DATE, PAY_DATE EVENT_DATE
...
SELECT FROM_UNIXTIME(unix_timestamp(NQ_CREATE_DATE), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9)  MSISDN,
    ...
    'ADJUSTMENT CHARGE ' ||upper(USAGE_DESCRIPTION)||' '||a.channel_id type, current_timestamp() INSERT_DATE, CREATE_DATE EVENT_DATE
