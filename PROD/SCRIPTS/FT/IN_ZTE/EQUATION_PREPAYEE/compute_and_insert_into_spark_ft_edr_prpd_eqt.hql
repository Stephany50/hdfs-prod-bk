INSERT INTO TABLE MON.SPARK_FT_EDR_PRPD_EQT PARTITION(EVENT_DATE)
SELECT FROM_UNIXTIME(unix_timestamp(START_TIME)+3600, 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(BILLING_NBR, -9) MSISDN,
    ( CASE WHEN ACCT_RES_ID1 = 1 AND CHARGE1 >0 THEN NVL(CHARGE1, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID2 = 1 AND CHARGE2 >0 THEN NVL(CHARGE2, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID3 = 1 AND CHARGE3 >0 THEN NVL(CHARGE3, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID4 = 1 AND CHARGE4 >0 THEN NVL(CHARGE4, 0) ELSE 0 END
    )/100  MAIN_DEBIT,
    ( CASE WHEN ACCT_RES_ID1 = 20 AND CHARGE1 >0 THEN NVL(CHARGE1, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID2 = 20 AND CHARGE2 >0 THEN NVL(CHARGE2, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID3 = 20 AND CHARGE3 >0 THEN NVL(CHARGE3, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID4 = 20 AND CHARGE4 >0 THEN NVL(CHARGE4, 0) ELSE 0 END
    )/100  LOAN_DEBIT,
    ( CASE WHEN ACCT_RES_ID1 = 21 AND CHARGE1 >0 THEN NVL(CHARGE1, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID2 = 21 AND CHARGE2 >0 THEN NVL(CHARGE2, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID3 = 21 AND CHARGE3 >0 THEN NVL(CHARGE3, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID4 = 21 AND CHARGE4 >0 THEN NVL(CHARGE4, 0) ELSE 0 END
    )/100 SASSAYE_DEBIT,
    ( CASE WHEN ACCT_RES_ID1 = 1 AND CHARGE1 <0 THEN -NVL(CHARGE1, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID2 = 1 AND CHARGE2 <0 THEN -NVL(CHARGE2, 0) ELSE 0 END
        + CASE  WHEN ACCT_RES_ID3 = 1 AND CHARGE3 <0 THEN -NVL(CHARGE3, 0) ELSE 0 END
    ...
    CASE WHEN NVL(CHARGE1, 0) >=0 or NVL(CHARGE2, 0) >=0 or NVL(CHARGE3, 0) >=0 or NVL(CHARGE4, 0) >=0 then 'CONSO DATA' else 'CONSO DATA ACTIVATION' END TYPE, current_timestamp() INSERT_DATE, START_DATE EVENT_DATE
FROM CDR.SPARK_IT_ZTE_DATA A
...
GROUP BY START_DATE, START_TIME, CALLING_NBR, BILLING_IMSI, CALLING_IMEI, CALLED_NBR, BILLING_NBR,
...
SELECT FROM_UNIXTIME(unix_timestamp(START_TIME)+3600, 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(BILLING_NBR, -9) MSISDN,
    ...
    case when NVL(CHARGE1, 0) >=0 or NVL(CHARGE2, 0) >=0 or NVL(CHARGE3, 0) >=0 or NVL(CHARGE4, 0) >=0 then 'CONSO VOIX SMS' else 'CONSO VOIX SMS ACTIVATION' END TYPE, current_timestamp() INSERT_DATE, START_DATE EVENT_DATE
...
GROUP BY START_DATE, START_TIME, CALLING_NBR, BILLING_IMSI, CALLING_IMEI, CALLED_NBR, BILLING_NBR,
    ...
    PRICE_ID1, PRICE_ID2, PRICE_ID3, PRICE_ID4, ACCT_ITEM_TYPE_ID1, ACCT_ITEM_TYPE_ID2, ACCT_ITEM_TYPE_ID3, ACCT_ITEM_TYPE_ID4
UNION ALL
SELECT  FROM_UNIXTIME(unix_timestamp(NQ_CREATEDDATE), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9) MSISDN,
    CASE WHEN HybIPP.OFFER_CODE is not null THEN 0 ELSE MAIN_DEBIT/100 END MAIN_DEBIT, LOAN_DEBIT/100 LOAN_DEBIT , SASSAYE_DEBIT,
    ...
    WHEN HybIPP.OFFER_CODE is not null or lower(BENEFIT_NAME) like "%ipp%benefit%"  THEN IF(lower(BENEFIT_NAME) like '%flex%', 'SUBSCRIPTION DEPOT FLEX', IF(lower(BENEFIT_NAME) like '%benefit%', 'SUBSCRIPTION DEPOT BENEFIT', 'SUBSCRIPTION DEPOT HYBRID'))
    ELSE 'SUBSCRIPTION' END TYPE, current_timestamp() INSERT_DATE, CREATEDDATE EVENT_DATE --, BENEFIT_NAME
FROM
    (SELECT A.*,
    IF(CAST(SPLIT(account_res_id , ',')[0] AS INT)=1, CAST(SPLIT(EVENT_COST_CHAR, ',')[0] AS INT),0)
    + IF(CAST(SPLIT(account_res_id , ',')[1] AS INT)=1,CAST(IF(SPLIT(EVENT_COST_CHAR, ',')[1] is null,'0',SPLIT(EVENT_COST_CHAR, ',')[1]) AS INT),0)
    + IF(CAST(SPLIT(account_res_id , ',')[2] AS INT)=1,CAST(IF(SPLIT(EVENT_COST_CHAR, ',')[2] is null,'0',SPLIT(EVENT_COST_CHAR, ',')[2]) AS INT),0)
    + IF(account_res_id is null ,EVENT_COST_SUM,0)  MAIN_DEBIT,

    IF(CAST(SPLIT(account_res_id , ',')[0] AS INT)=20, CAST(SPLIT(EVENT_COST_CHAR, ',')[0] AS INT),0)
    + IF(CAST(SPLIT(account_res_id , ',')[1] AS INT)=20, CAST(IF(SPLIT(EVENT_COST_CHAR, ',')[1] is null,'0',SPLIT(EVENT_COST_CHAR, ',')[1]) AS INT),0)
    + IF(CAST(SPLIT(account_res_id , ',')[2] AS INT)=20, CAST(IF(SPLIT(EVENT_COST_CHAR, ',')[2] is null,'0',SPLIT(EVENT_COST_CHAR, ',')[2]) AS INT),0) LOAN_DEBIT,

    IF(CAST(SPLIT(account_res_id , ',')[0] AS INT)=21, CAST(SPLIT(EVENT_COST_CHAR, ',')[0] AS INT),0)
    + IF(CAST(SPLIT(account_res_id , ',')[1] AS INT)=21,CAST(IF(SPLIT(EVENT_COST_CHAR, ',')[1] is null,'0',SPLIT(EVENT_COST_CHAR, ',')[1]) AS INT),0)
    + IF(CAST(SPLIT(account_res_id , ',')[2] AS INT)=21,CAST(IF(SPLIT(EVENT_COST_CHAR, ',')[2] is null,'0',SPLIT(EVENT_COST_CHAR, ',')[2]) AS INT),0) SASSAYE_CREDIT
    ...
    FROM (
        ...
        - CASE WHEN CAST(SPLIT(BEN_BAL, '&')[0] AS INT)=21 THEN CAST(SPLIT(BEN_BAL, '&')[1] AS INT) ELSE 0 END SASSAYE_DEBIT
        FROM ( SELECT * FROM CDR.SPARK_IT_ZTE_SUBSCRIPTION WHERE CREATEDDATE = '###SLICE_VALUE###') A
        LATERAL VIEW EXPLODE(SPLIT(NVL(BENEFIT_BAL_LIST, ''), '#')) TMP AS BEN_BAL
    ) A ) A
LEFT JOIN (SELECT OFFER_NAME, MAX(OFFER_ID) OFFER_ID, MAX(OFFER_CODE) OFFER_CODE FROM DIM.DT_IPP_HYBRID GROUP BY OFFER_NAME ) HybIPP ON NVL(A.PRICE_PLAN_CODE, '1000000')  = HybIPP.OFFER_CODE
GROUP BY NQ_CREATEDDATE, CREATEDDATE, ACC_NBR, PREPAY_FLAG, PROD_SPEC_CODE, CHANNEL_ID, EVENT_COST, HybIPP.OFFER_CODE,
    PRICE_PLAN_CODE, RELATED_PROD_CODE, SUBS_EVENT_ID, BENEFIT_NAME, BENEFIT_BAL_LIST, OLD_SUBS_STATE, NEW_SUBS_STATE,MAIN_DEBIT,LOAN_DEBIT,SASSAYE_DEBIT
UNION ALL
SELECT FROM_UNIXTIME(unix_timestamp(PAY_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9) MSISDN,
    ...
    CASE WHEN CHANNEL_ID=9 THEN 'RECHARGE SOS DATA ' else 'RECHARGE ' end ||upper(nvl(USAGE_DESCRIPTION, 'UNKNOW'))||' '||a.channel_id TYPE, current_timestamp() INSERT_DATE, PAY_DATE EVENT_DATE
FROM CDR.SPARK_IT_ZTE_RECHARGE a left join DIM.DT_ZTE_USAGE_TYPE b on a.channel_id=b.USAGE_CODE
WHERE PAY_DATE = '###SLICE_VALUE###'
    and ACCT_RES_CODE IN ( 1, 20, 21)
    AND  BILL_AMOUNT!= 0
GROUP BY PAY_TIME, PAY_DATE, PAYMENT_ID, ACC_NBR, PREPAY_FLAG, CHANNEL_ID, PAYMENT_METHOD, ACCT_RES_CODE, BENEFIT_NAME, BILL_AMOUNT, LOAN_AMOUNT, COMMISSION_AMOUNT, USAGE_DESCRIPTION
    UNION ALL
SELECT FROM_UNIXTIME(unix_timestamp(PAY_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9) MSISDN,
...
UNION ALL
SELECT FROM_UNIXTIME(unix_timestamp(CONCAT(TRANSACTION_DATE, ' ',TRANSACTION_TIME), 'yyyy-MM-dd HHmmss' ), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(MSISDN, -9)  MSISDN,
     FEE/100 MAIN_DEBIT,  0 LOAN_DEBIT, 0 SASSAYE_DEBIT,
    0 MAIN_CREDIT, 0 LOAN_CREDIT, 0 SASSAYE_CREDIT,
    'SOS DATA FEE' TYPE, current_timestamp() INSERT_DATE, TRANSACTION_DATE EVENT_DATE
FROM CDR.SPARK_IT_ZTE_EMERGENCY_DATA
WHERE TRANSACTION_DATE = '###SLICE_VALUE###'
    AND FEE>0 AND TRANSACTION_TYPE='PAYBACK'
GROUP BY TRANSACTION_TIME, TRANSACTION_DATE, MSISDN, AMOUNT, TRANSACTION_TYPE, FEE, CONTACT_CHANNEL
UNION ALL
SELECT TRANSACTION_DATE EVENT_TIME, SUBSTR(MSISDN, -9)  MSISDN,
    CASE WHEN TRANSACTION_TYPE= 'LOAN' AND AMOUNT>0 THEN AMOUNT
        WHEN TRANSACTION_TYPE= 'PAYBACK' AND AMOUNT<0 THEN - AMOUNT
        ELSE AMOUNT END /100 MAIN_DEBIT,  0 LOAN_DEBIT, 0 SASSAYE_DEBIT,
    0 MAIN_CREDIT, 0 LOAN_CREDIT, 0 SASSAYE_CREDIT,
    CASE WHEN TRANSACTION_TYPE= 'LOAN' THEN 'SOS VOICE LOAN'
        WHEN TRANSACTION_TYPE= 'PAYBACK' THEN 'SOS VOICE PAYBACK'
    ELSE 'SOS VOICE' END TYPE, current_timestamp() INSERT_DATE, TRANSACTION_DATE EVENT_DATE
FROM CDR.SPARK_IT_ZTE_LOAN_CDR
WHERE TO_DATE(TRANSACTION_DATE) = '###SLICE_VALUE###'
    AND  AMOUNT!= 0
GROUP BY TRANSACTION_DATE, MSISDN, AMOUNT, TRANSACTION_TYPE, FEE, CONTACT_CHANNEL
UNION ALL
SELECT TRANSACTION_DATE EVENT_TIME, SUBSTR(MSISDN, -9)  MSISDN,
     FEE/100 MAIN_DEBIT,  0 LOAN_DEBIT, 0 SASSAYE_DEBIT,
    0 MAIN_CREDIT, 0 LOAN_CREDIT, 0 SASSAYE_CREDIT,
    'SOS VOICE FEE' TYPE, current_timestamp() INSERT_DATE, TRANSACTION_DATE EVENT_DATE
FROM CDR.SPARK_IT_ZTE_LOAN_CDR
WHERE TO_DATE(TRANSACTION_DATE) = '###SLICE_VALUE###'
    AND FEE>0 AND TRANSACTION_TYPE='PAYBACK'
GROUP BY TRANSACTION_DATE, MSISDN, AMOUNT, TRANSACTION_TYPE, FEE, CONTACT_CHANNEL
...
SELECT FROM_UNIXTIME(unix_timestamp(NQ_CREATE_DATE), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9)  MSISDN,
    ...
    'ADJUSTMENT CHARGE ' ||upper(nvl(USAGE_DESCRIPTION, 'UNKNOW'))||' '||a.channel_id type, current_timestamp() INSERT_DATE, CREATE_DATE EVENT_DATE
FROM CDR.SPARK_IT_ZTE_ADJUSTMENT a left join DIM.DT_ZTE_USAGE_TYPE b on a.channel_id=b.USAGE_CODE
WHERE CREATE_DATE = '###SLICE_VALUE###'
    and ACCT_RES_CODE IN ( 1, 20, 21)
    and  CHARGE != 0
GROUP BY NQ_CREATE_DATE, CREATE_DATE, ACC_NBR, ACCT_BOOK_ID, ACCT_RES_CODE, CHARGE, PRE_REAL_BALANCE, USAGE_DESCRIPTION, channel_id
    UNION ALL
SELECT FROM_UNIXTIME(unix_timestamp(NQ_CREATE_DATE), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9)  MSISDN,
...
GROUP BY NQ_CREATE_DATE, CREATE_DATE, ACC_NBR, ACCT_BOOK_ID, ACCT_RES_CODE, CHARGE, COMMISSION_AMOUNT, LOAN_AMOUNT, PRE_REAL_BALANCE, channel_id
UNION ALL
...
SELECT FROM_UNIXTIME(unix_timestamp(EVENT_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9) MSISDN, 0 MAIN_DEBIT, 0 LOAN_DEBIT, 0 SASSAYE_DEBIT,
  ...
  CASE WHEN upper(b.price_plan_name) like '%FLEX%' THEN 'RECURRING FLEX' ELSE NVL('RECURRING '||upper(b.price_plan_name), 'RECURRING') END type, current_timestamp() insert_date, EVENT_DATE
FROM (
