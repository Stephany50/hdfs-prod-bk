...
SELECT FROM_UNIXTIME(unix_timestamp(START_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(BILLING_NBR, -9) MSISDN,
    ...
    CASE WHEN NVL(CHARGE1, 0) >=0 or NVL(CHARGE2, 0) >=0 or NVL(CHARGE3, 0) >=0 or NVL(CHARGE4, 0) >=0 then 'CONSO DATA' else 'CONSO DATA ACTIVATION' END TYPE, current_timestamp() INSERT_DATE, cast(START_TIME as date) EVENT_DATE
...
SELECT FROM_UNIXTIME(unix_timestamp(START_TIME), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(BILLING_NBR, -9) MSISDN,
    ...
    case when NVL(CHARGE1, 0) >=0 or NVL(CHARGE2, 0) >=0 or NVL(CHARGE3, 0) >=0 or NVL(CHARGE4, 0) >=0 then 'CONSO VOIX SMS' else 'CONSO VOIX SMS ACTIVATION' END TYPE, current_timestamp() INSERT_DATE, cast(START_TIME as date) EVENT_DATE
