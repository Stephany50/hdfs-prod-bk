...
SELECT  FROM_UNIXTIME(unix_timestamp(NQ_CREATEDDATE), 'yyyy-MM-dd HH:mm:ss') EVENT_TIME, SUBSTR(ACC_NBR, -9) MSISDN,
    ...
    WHEN HybIPP.OFFER_CODE is not null THEN IF(lower(BENEFIT_NAME) like '%flex%', 'SUBSCRIPTION DEPOT FLEX', IF(lower(BENEFIT_NAME) like '%benefit%', 'SUBSCRIPTION DEPOT BENEFIT', 'SUBSCRIPTION DEPOT HYBRID'))
