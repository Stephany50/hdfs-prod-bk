INSERT INTO MON.SPARK_FT_CBM_ADVANCED_DATA_TRAFIC_FDD_DAILY
    SELECT DISTINCT 
        A.imei,
        A.tac_code,
        A.terminal_brand,
        A.terminal_model,
        F.type_box AS box_model,
    ...
    (
        SELECT * 
        FROM DIM.CBM_REF_MBB_ROUTING
    ) F 
    ON A.tac_code = F.tac_code
    LEFT JOIN
    (
        SELECT DISTINCT
            imei,
