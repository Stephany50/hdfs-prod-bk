...
LEFT JOIN (SELECT DISTINCT ORIGINAL_FILE_NAME FROM CDR.IT_OMNY_SERVICES_CHARGES_DETAILS B WHERE TRANSACTION_DATE = '###SLICE_VALUE###' and TO_DATE(ORIGINAL_FILE_DATE) = '###SLICE_VALUE###') B ON SUBSTRING(B.ORIGINAL_FILE_NAME,-12,8) = SUBSTRING(C.FILE_NAME,-12,8)
WHERE B.ORIGINAL_FILE_NAME IS NULL
UNION
...
WHERE B.ORIGINAL_FILE_NAME IS NULL
UNION
SELECT C.* FROM (SELECT 'BDI' TABLE_SOURCE,'NORMAL' FLUX_TYPE,'BDI' FLUX_NAME,CONCAT('bdi','_',DATE_FORMAT('###SLICE_VALUE###','YYYYMMdd'),'.','csv') FILE_NAME, CURRENT_TIMESTAMP INSERT_DATE,'###SLICE_VALUE###' ORIGINAL_FILE_DATE )C
