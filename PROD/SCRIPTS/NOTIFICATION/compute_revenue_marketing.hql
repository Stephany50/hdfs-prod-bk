...
from (
    SELECT * FROM (
        ...
        WHERE TRAFFIC_MEAN='REVENUE' AND SOURCE_DATA NOT LIKE '%POST%'  and f.OPERATOR_CODE = 'OCM' and SUB_ACCOUNT = 'MAIN' AND f.TRANSACTION_DATE = '###SLICE_VALUE###'
    )T1
    LEFT JOIN (
        ...
        FROM  REPORT.FT_GLOBAL_ACTIVITY_DAILY f, dim.dt_usages
        WHERE TRAFFIC_MEAN='REVENUE' AND SOURCE_DATA NOT LIKE '%POST%'  and f.OPERATOR_CODE = 'OCM' and SUB_ACCOUNT = 'MAIN' AND f.TRANSACTION_DATE   = DATE_SUB('###SLICE_VALUE###',1)
...
join (
    ...
    WHERE TRAFFIC_MEAN='REVENUE' AND SOURCE_DATA NOT LIKE '%POST%' 
