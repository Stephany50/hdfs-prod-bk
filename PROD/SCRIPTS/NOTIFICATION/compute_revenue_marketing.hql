...
from (
    SELECT * FROM (
        ...
        WHERE TRAFFIC_MEAN='REVENUE'  and f.OPERATOR_CODE = 'OCM' and SUB_ACCOUNT = 'MAIN' AND f.TRANSACTION_DATE = '2019-08-08'
    )T1
    LEFT JOIN (
        SELECT '2019-08-08' TRANSACTION_DATE,
        SUM(case
            when service_code = usage_code AND UPPER(USAGE_DESCRIPTION) = 'VOIX'  then TAXED_AMOUNT
            else 0
        end) VOICE_PAYGO_yd,
        ...
        FROM  AGG.FT_GLOBAL_ACTIVITY_DAILY f, dim.dt_usages
        WHERE TRAFFIC_MEAN='REVENUE'  and f.OPERATOR_CODE = 'OCM' and SUB_ACCOUNT = 'MAIN' AND f.TRANSACTION_DATE   = DATE_SUB('2019-08-08',1)
...
join (
    ...
    select max(transaction_date) transaction_date,
        sum(case when transaction_date = '2019-08-08' then main_rated_amount
            else 0
        end) ca_roaming_out,
        sum(case when transaction_date = DATE_SUB('2019-08-08',1) then main_rated_amount
            else 0
        end) ca_roaming_out_yd
    from AGG.FT_GSM_TRAFFIC_REVENUE_DAILY
    where transaction_date   between DATE_SUB('2019-08-08',1) and '2019-08-08' and destination like '%ROAM%'
) b on b.transaction_date = a.transaction_date
join (
    select
    max(TRANSACTION_DATE) transaction_date,
    SUM(case when transaction_date = '2019-08-08' then TAXED_AMOUNT
            else 0
        end) TOTAL_AMOUNT,
    SUM(case when transaction_date = DATE_SUB('2019-08-08',1)  then TAXED_AMOUNT
            else 0
        end) TOTAL_AMOUNT_yd
    ...
    WHERE TRAFFIC_MEAN='REVENUE' 
        ...
        AND e.TRANSACTION_DATE   between DATE_SUB('2019-08-08',1)and '2019-08-08'
) c on c.transaction_date = b.transaction_date
join (-- MTD
    select '2019-08-08'sdate,SUM(TAXED_AMOUNT)  MTD_AMOUNT
    ...
    where TRAFFIC_MEAN='REVENUE' 
        ...
        and SUB_ACCOUNT  In  ('MAIN')
        and SEGMENTATION  In  ('Staff','B2B','B2C')
        AND TRANSACTION_DATE between  CONCAT(SUBSTRING('2019-08-08',0,7),'-','01') and '2019-08-08'
) d on d.sdate = c.transaction_date
join (-- LMTD
    select
     '2019-08-08' sdate
    ,SUM(TAXED_AMOUNT)*CAST(SUBSTRING('2019-08-08',9,2) AS INT)/CAST(SUBSTRING(add_months('2019-08-08',-1),9,2) AS INT) LMTD_AMOUNT
    from  AGG.FT_GLOBAL_ACTIVITY_DAILY a
    LEFT JOIN DIM.DT_OFFER_PROFILES   ON PROFILE_CODE = upper(COMMERCIAL_OFFER_CODE)
    where TRAFFIC_MEAN='REVENUE' 
        and a.OPERATOR_CODE  In  ('OCM')
        and SUB_ACCOUNT  In  ('MAIN')
        and SEGMENTATION  In  ('Staff','B2B','B2C')
        AND TRANSACTION_DATE between  add_months(CONCAT(SUBSTRING('2019-08-08',0,7),'-','01'),-1)
        and add_months('2019-08-08',-1)
