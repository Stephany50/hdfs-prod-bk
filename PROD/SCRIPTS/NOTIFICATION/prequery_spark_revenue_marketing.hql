SELECT  IF(T_1.REVENUE_EXISTS = 0
    ...
    AND T_3.NB_INSERT=5
    AND ABS(T_4.VOICE_PAYGO/T_4.VOICE_PAYGO_avg-1)<=0.2
    AND ABS(T_4.VOICE_BUNDLE/T_4.VOICE_BUNDLE_avg-1)<=0.2 
    AND ABS(T_4.DATA_AMOUNT/T_4.DATA_AMOUNT_avg-1)<=0.2
    AND ABS(T_6.ca_roaming_out/T_6.ca_roaming_out_avg-1)<=0.9  
    AND ABS(T_4.SMS_AMOUNT/T_4.SMS_AMOUNT_avg-1)<=0.2
    AND ABS(T_4.CA_VAS_BRUT/T_4.CA_VAS_BRUT_avg-1)<=0.2  
    AND ABS(T_5.total_amount/T_5.TOTAL_AMOUNT_avg-1)<=0.2  
