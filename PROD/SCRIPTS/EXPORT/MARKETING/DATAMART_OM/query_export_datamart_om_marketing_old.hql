...
LEFT JOIN (SELECT SITE_NAME, MAX(SECTEUR_OM) SECTEUR ,MAX(REGION) REGION , MAX(ZONEPMO) ZONEPMO FROM DIM.DT_GSM_CELL_CODE GROUP BY SITE_NAME) B ON NVL(UPPER(A.SITE_NAME),'ND')=NVL(UPPER(B.SITE_NAME),'ND')
