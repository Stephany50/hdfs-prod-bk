...
LEFT JOIN (SELECT * FROM FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE=(SELECT MAX(EVENT_DATE) FROM MON.SPARK_FT_CLIENT_LAST_SITE_DAY WHERE EVENT_DATE between date_sub('###SLICE_VALUE###', 7) AND '###SLICE_VALUE###')) B ON A.ACCOUNT_NUMBER = B.MSISDN;
