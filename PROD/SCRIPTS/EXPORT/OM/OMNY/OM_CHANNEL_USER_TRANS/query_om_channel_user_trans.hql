...
SENDER_CATEGORY_CODE SENDER_CATEGORY_CODE,
RECEIVER_MSISDN_ACC RECEIVER_MSISDN_ACC,
RECIEVER_CATEGORY_CODE RECIEVER_CATEGORY_CODE,
TRANSACTION_AMOUNT TRANSACTION_AMOUNT,
SERVICE_TYPE SERVICE_TYPE,
TRANSFER_STATUS TRANSFER_STATUS,
SENDER_PRE_BAL SENDER_PRE_BAL,
SENDER_POST_BAL SENDER_POST_BAL,
RECEIVER_PRE_BAL RECEIVER_PRE_BAL,
RECEIVER_POST_BAL RECEIVER_POST_BAL,
TRANSFER_ID TRANSFER_ID,
INSERT_DATE INSERT_DATE
FROM
(SELECT
 DATE_FORMAT(TRANSFER_DATETIME, 'yyyy-MM-dd') TRANSFER_DATE,
 ...
 DATE_FORMAT(TRANSFER_DATETIME, 'yyyy-MM-dd') EVENT_DATE

FROM
(SELECT * FROM CDR.SPARK_IT_OMNY_TRANSACTIONS WHERE TRANSFER_DATETIME = '###SLICE_VALUE###' AND SERVICE_TYPE IN ('CASHIN','OPTW') ) a
...
FROM
(SELECT * FROM CDR.SPARK_IT_OMNY_TRANSACTIONS WHERE TRANSFER_DATETIME = '###SLICE_VALUE###' AND SERVICE_TYPE IN ('O2C','CASHOUT','COUTBYCODE') ) a
...
(SELECT *
    ...
    WHERE ORIGINAL_FILE_DATE=(SELECT MAX(ORIGINAL_FILE_DATE) FROM CDR.SPARK_IT_OM_ALL_BALANCE WHERE ORIGINAL_FILE_DATE BETWEEN DATE_SUB('###SLICE_VALUE###', 7) AND '###SLICE_VALUE###') AND USER_DOMAIN IN ('TOTAL MARCHAND' ,'Total Partner' ,'Key Account Partner1' ,'Key Account Partner2')

 ) b

ON  a.RECEIVER_MSISDN = b.ACCOUNT_ID ) R
