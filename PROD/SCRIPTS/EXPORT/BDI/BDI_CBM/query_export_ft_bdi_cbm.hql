...
left join (select msisdn, est_conforme from MON.SPARK_FT_KYC_BDI_FLOTTE where event_date=DATE_SUB('###SLICE_VALUE###')) flotte on a.msisdn=flotte.msisdn
left join (select msisdn, conforme_art from mon.spark_ft_kyc_bdi_pp where event_date=DATE_SUB('###SLICE_VALUE###')) pp on a.msisdn=pp.msisdn
