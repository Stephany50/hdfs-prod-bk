...
FROM
(SELECT COUNT(distinct event_date) nb_dates FROM mon.spark_ft_nuran_local_traffic_day WHERE EVENT_DATE BETWEEN concat('###SLICE_VALUE###', '-01') and last_day(concat('###SLICE_VALUE###', '-01'))) T_1,
