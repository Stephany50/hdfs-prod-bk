...
FROM
(SELECT COUNT(distinct event_date) nb_dates FROM AGG.SPARK_FT_A_TRAFFIC_SORTANT_NURAN WHERE EVENT_DATE BETWEEN concat('###SLICE_VALUE###', '-01') and last_day(concat('###SLICE_VALUE###', '-01'))) T_1,
