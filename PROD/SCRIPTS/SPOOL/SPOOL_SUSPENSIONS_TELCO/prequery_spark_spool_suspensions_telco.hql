SELECT IF(T_1.SPOOL_COUNT = 0 AND T_1_PREV.SPOOL_PREV >= 10 AND
          ...
          AND C.nb_loc >= 10 AND D.nb_zsmart >= 10 AND E.nb_cont >= 10,"OK","NOK") FT_EXIST
FROM
    ...
    (select count(*) nb_cont from CDR.SPARK_IT_CONT where ORIGINAL_FILE_DATE=date_add('###SLICE_VALUE###',1)) E
